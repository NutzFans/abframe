	...
	<form id="form1" method="post">
				...
				<table id="goaltable" style="table-layout:fixed;border-collapse:collapse;word-wrap:break-word;;width:90%" bordercolor="#A8A8A8" border="1" align="center">	
					...
					<tr>
						<td align="center" colspan="8">
							<input name="ver.veriregoal" id="veriregoal" class="nui-textarea" style = "width: 100%;height:100px" maxLength="200" required="false">
						</td>
					</tr>
				</table>
			</div>
		...
		<fieldset id="field1" style="border:solid 1px #aaa;padding:3px;display:true">
			...
			<span style="color:red;;left:65px; display:block;position:relative;width:950px;" align="left">
				说明：试用期考核评估分数达到60分以上（含），将视作符合新员工录用条件之一。对于学历验证状态是“民教网已验证”、“2000年前学历无法验证”和“非正常学历”的员工，请务必关注其能力是否与岗位要求相匹配，请慎重评估。
			</span>
			<div style="padding:5px;">
				<input name="omAppraiseByTutor.appraiseid" id="appraiseid"  class="nui-hidden"/>
				<input name="omAppraiseByTutor.appraisertype" id="appraisertype"  class="nui-hidden"/>
				<input name="omAppraiseByTutor.appraiser" id="appraiser"  class="nui-hidden"/>
				<table style="table-layout:fixed;" >	
					<tr>
						<td align="right" style="width:100px">是否同意转正：</td>
						<td style="width:60px">
							...
							<input class="nui-hidden" name="omAppraiseByTutor.appraisestatus" id="appraisestatus"/>
						</td>
					...
					<tr>
						...
						<td colspan="2">
							<input name="omAppraiseByTutor.appraise" id="appraise" class="nui-textarea" style = "width: 500px;height:50px" maxLength="200" required="true">
						</td>
					</tr>
				</table>
			</div>
...
<script type="text/javascript">
  		...
  		nui.ajax({
            ...
            success: function (o) {
            	...
            	if(data.ver){
            		...
            		form1.setData(data);
            		changeGrade();
            		nui.get("appraisertype").setValue("1");
            		shoulddate = o.ver.shoulddate;
			...
			for(var i=0;i<omIntendPeriod.length;i++){
				...
				if(omIntendPeriod[i].omIntendScores){
					for(var j=0;j<omIntendPeriod[i].omIntendScores.length;j++){
						if(j>0){
							//插入一行
					    	var myNewRow = myTable.insertRow(myTable.rows.length-3);
					    	//取得表格的总行数
					    	var aRows=myTable.rows;
						}
						//取得表格的总网格数
				    	...
				    	oCell1_44.innerHTML = "<input class='nui-spinner' name='omAppraiseByTutordet["+y+"].grade' id='grade"+y+"'  style='width:90%' required='true' format='n2' showbutton='false' changeOnMousewheel='false' minValue='0' maxValue='100' onvaluechanged='changeGrade'/>"
				...
				if(nownum == 0){
					...
					oCell1_5.innerHTML = "<input class='nui-spinner' name='omAppraiseByTutordet["+y+"].grade' id='grade"+y+"'  style='width:90%' required='true' format='n2' showbutton='false' changeOnMousewheel='false' minValue='0' maxValue='100' onvaluechanged='changeGrade'/>"
										+"<input class='nui-hidden' name='omAppraiseByTutordet["+y+"].scoreid' id='scoreid"+y+"'/>"
										+"<input class='nui-hidden' name='omAppraiseByTutordet["+y+"].goalid' id='goalid"+y+"'/>"
	...
	function changeappraisestatus(){
		...
		nui.get("appraisestatus").setValue(verirestatue);
				...
				nui.ajax({
	    			...
	    			success:function(action){
	    				...
	    				if(action.exception == null){
    						...
    						nui.alert("提交转正流程成功！但通知邮件发送失败，请手动联系！","系统提示",function(){
				        		CloseWindow("ok");
			    				nui.get("appButton").setEnabled(true);
				        	});
	    				}else{
