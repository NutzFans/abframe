...
<div class="nui-fit">
	...
	<form id="form1" method="post">
		...
		<fieldset id="field1" style="border:solid 1px #aaa;padding:3px;display:true">
			<legend>转正意见</legend>
			<span style="color:red;;left:65px; display:block;position:relative;width:1050px;" align="left">
				...
				<table style="table-layout:fixed;" >
					<tr>
						...
						<td  style="width:120px">
							<input name="ver.verirestatue" id="verirestatue" class="nui-dictcombobox" dictTypeId="VERI_STATUS" width="100%" required="true" shownullItem="false" onvaluechanged='changeappraisestatus' readOnly='readOnly'/>
						...
						<td  style="width:250px">
		                	...
		                	<table style="display:none;" id="showturn1">
								<tr>
									<td align="right">提前至
										<input name="ver.realdate" id="realdate" class="nui-datepicker" required="true" style="width: 100px;"/>
										日成为正式员工
									</td>
								</tr>
		                	</table>
					...
					<tr>
						<td align="right" style="width:130px">考试结果：</td>
						<td colspan="2" id="testresult"></td>
					</tr>
				</table>
			</div>
		</fieldset>
...
<div style="text-align:center;padding:10px;" class="nui-toolbar">
	<%--<a class="nui-button" onclick="onOk('1')" style="width:60px;margin-right:20px;" id="appButton">提交</a>--%>
	<a class="nui-button"onclick="onCancel" iconCls="icon-close" style="width:60px;">关闭</a>
	<a class="nui-button" onclick="doViewSettle" style="width:80px;margin-right:20px;" id="appButton1">查看流程图</a>
	<%--<a class="nui-button" onclick="onOk('2')" style="width:80px;margin-right:20px;" id="appButton2">终止流程</a>--%>
</div>
</body>
	...
	initdate();
  	function initdate(){
  		nui.ajax({
            ...
            data: {workItemID:null,processInstID:processinstid},
            type: 'POST',
            ...
            success: function (o) {
            	...
            	if(data.ver){
            		...
            		document.getElementById("setoldDate").innerHTML = o.ver.shoulddate == null?"":(o.ver.shoulddate+"日成为正式员工");
			...
			for(var i=0;i<omIntendPeriod.length;i++){
				...
				if(omIntendPeriod[i].omIntendScores){
					for(var j=0;j<omIntendPeriod[i].omIntendScores.length;j++){
				    	...
				    	oCell1_55.innerHTML = data.omAppraisedet[y].grade== null ?"":data.omAppraisedet[y].grade;
				    	
				    	sumgrade1 = sumgrade1+(data.omAppraisedet[y].grade==null?0:(data.omAppraisedet[y].grade*omIntendPeriod[i].omIntendScores[j].weight*omIntendPeriod[i].weight));
				    	sumgrade = sumgrade+(data.omAppraiseByTutordet[y].grade==null?0:(data.omAppraiseByTutordet[y].grade*omIntendPeriod[i].omIntendScores[j].weight*omIntendPeriod[i].weight));
				    	
				    	oCell1_44.innerHTML = data.omAppraiseByTutordet[y].grade== null ?"":data.omAppraiseByTutordet[y].grade;
				...
				if(nownum == 0){
					...
					oCell1_6.innerHTML = data.omAppraisedet[y].grade== null ?"":data.omAppraisedet[y].grade;
					
					sumgrade1 = sumgrade1+(data.omAppraisedet[y].grade== null ?0:(data.omAppraisedet[y].grade*omIntendPeriod[i].weight));
					sumgrade = sumgrade+(data.omAppraiseByTutordet[y].grade == null ?0:(data.omAppraiseByTutordet[y].grade*omIntendPeriod[i].weight));
					
					var oCell1_5=aRows[myNewRow.rowIndex].insertCell(aCells.length);
					oCell1_5.align="center";
					oCell1_5.innerHTML = data.omAppraiseByTutordet[y].grade== null ?"":data.omAppraiseByTutordet[y].grade;
