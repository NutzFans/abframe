			...
			<div style="padding:5px;">
				...
				<table style="table-layout:fixed;" id="table_entry1">
					...
					<tr>
                		...
                		<td width="190px">    
							<input property="editor" name="omIntendJoin.assistant" id="assistant" width="100%"  class="nui-buttonedit" onbuttonclick="selectTutor(this)" showClose="false" required="true" allowInput="false" />
	                    </td>
					...
					<tr>
                		...
                		<td width="190px">    
	                    	<input name="omIntendJoin.isSM" id="isSM" width="100%" class="nui-dictcombobox" dictTypeId="MIS_YN" required="true"/>
	                    </td>
...
<div style="text-align:center;padding:10px;" class="nui-toolbar">
	...
	<a class="nui-button" onclick="onOk()" iconCls="icon-organization" style="width:100px;margin-right:20px;" id="appButton">提交流程</a>
	<a class="nui-button" onclick="onOk(0)" style="width:80px;margin-right:20px;" id="breakProcess">中止流程</a>
...
<script type="text/javascript">
       	...
       	nui.ajax({
            ...
            success: function (o) {
            	if(o.omIntendJoin){
            		...
            		if(o.omIntendJoin.spr == null || o.omIntendJoin.spr == ''){
            			changeintendworkplace();
            		}else{
	            		nui.get("spr").setValue(o.omIntendJoin.spr);
		                nui.get("spr").setText(o.omIntendJoin.sprname);
            		}
            		...
            		changeactdate1();
            		//初始化合同签署部门
            		var htorgname1 = o.omIntendJoin.htorgname;
				  	if(htorgname1){
				  		if(htorgname1.indexOf(',')!=-1){
				  			var orgid = htorgname1.split(',')[1];
				  			nui.get("htorgname1").setValue(orgid);
				  		}
				  	}
            		//附件加载
	...
	function onOk(e){
    	...
    	for(var j = 0;j < filePaths;j++){
			...
			if(a == 0){
				nui.alert("新增新员工入职附件不能为空！");
				return false;
			}
    	}
    	if(!form_Intend.validate() && e!=0){
			nui.alert("信息录入不完整!");
			return;
		}else{
    		operate = e;
	...
	function SaveData1(){
		...
		nui.confirm(tip+"您确定要"+message+"该流程吗？","操作提示",function (action){
			if(action == "ok"){
				nui.get("appButton").setEnabled(false);
				nui.get("breakProcess").setEnabled(false);
	    		...
	    		nui.ajax({
	    			...
	    			success:function(action){
	    				...
	    				if(action.exception == null){
	    					nui.alert("新员工入职确认流程"+message+"成功！","系统提示",function(){
				        		CloseWindow("ok");
			    				nui.get("appButton").setEnabled(true);
	        					nui.get("breakProcess").setEnabled(true);
				        	});
	    				}else if(action.exception == "fallsendmail"){
				        	nui.alert("新员工入职确认流程"+message+"成功！"+"但"+action.sendMessage+action.sendMail,"系统提示",function(){
				        		CloseWindow("ok");
			    				nui.get("appButton").setEnabled(true);
	        					nui.get("breakProcess").setEnabled(true);
				        	});
