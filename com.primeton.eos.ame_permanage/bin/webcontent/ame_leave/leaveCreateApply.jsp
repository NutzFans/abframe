...
<div class="nui-fit">
	<div class="nui-panel" title="请假申请" style="width: 100%;">
		...
		<fieldset id="field1" style="border: solid 1px #aaa;padding: 3px;width: 1000px">
			<legend>请假信息</legend>
				...
				<div style="padding: 5px;">
					<table style="table-layout: fixed;">
						<tr>
							...
							<td>
								<input id="orgid" name="orgid" class="nui-combobox" readonly="readonly" valueField="orgid" textField="orgname"  style="width: 140px" required="true"/>
							</td>
		...
		<fieldset id="field11" style="border: solid 1px #aaa;padding: 3px;width: 1000px;">
			<legend>请假详情</legend>
	    	...
	    	<div id="grid_leave" class="nui-datagrid" style="width: 100%;height: auto;" allowCellSelect="true" 
	            ...
	            <div property="columns">
	                ...
	                <div field="startdate" name="startdate" width="110" align="center" headerAlign="center" vtype="required:true" renderer="onDealDate">开始日期
		...
		<div><span style="color: red">请假审批规则：三天以内机构主管审批；三天以上（包括三天）七天以内机构主管+上级主管审批；七天以上（包括七天）机构主管+上级主管+再上级主管审批。</span></div>
		<div><span>项目组成员需要增加项目经理作为审批人，请选择附加审批人：</span><input id="spr" class="mini-textboxlist"  allowInput="true" textField="EMPNAME" valueField="USERID" searchField="name" style="width:150px;" url="com.primeton.eos.ame_common.ame_common.queryAllEmpByName.biz.ext"><!-- <a class = "nui-button" onclick="">选择附加审批人</a> --></div>
		...
		<fieldset id="field6" style="border:solid 1px #aaa;padding:3px;width: 1000px;">
				<legend>请假附件</legend>
		...
		<form id="opioionform" method="post" >
	        <input name="misOpinion.auditstatus"  class="nui-hidden" value="3" />
    	</form>
	</div>
</div>
<div style="text-align: center;padding: 10px;" class="nui-toolbar">
	<a class="nui-checkbox" style="width:100px;padding-left: 10px;float: left;" id="guanZhuButton">关注</a>
	<a class="nui-button" onclick="onOk('1')" iconCls="icon-leaveCreateApply2" id="creatLeave" style="width: 120px;margin-right: 20px;">发起请假申请</a>
	<a class="nui-button" onclick="onOk('0')" iconCls="icon-tempsave2" id="saveLeave" style="width: 100px;margin-right: 140px;">暂时保存</a>
</div>
</body>
<script type="text/javascript">
	...
	nui.get("userid").setValue("<%=userno %>");
  	...
  	addleave();//默认增加一行请假明细
	promptInit({"workItemID": 0,"processDefName": "com.primeton.eos.ame_permanage.ame_leave","activityDefID": "manualActivity"});
    ...
    function onOk(e){
    	...
    	for(var j = 0;j < filePaths;j++){
    		var a = document.getElementsByName("uploadfile")[j].files.length;
    		if(a == 0){
    			nui.alert("新增附件不可以为空");
    			return false;
    		}
    	...
    	if(e == "1"){//发起
	    	...
	    	nui.confirm("提醒：请假申请邮件通知要抄送给部门助理。请确认是否提交流程？","操作提示",function(o){
	        	if("ok" == o){
	        		type = 1;
					document.getElementById("fileCatalog").value="ameLeave";
		        	form2.submit();
			     }
	        });
    	}else if(e == "0"){//暂存
    ...
    function SaveData(){
		...
		leaveinfo.totaldays = $(grid_leave.getSummaryCellEl("days")).find("b").text();//总天数
    	...
    	nui.ajax({	
      		url: "com.primeton.eos.ame_permanage.ame_leavepro.creatLeave.biz.ext",
	      	type: 'POST',
          	data: json,
      		success: function (text) {
      			nui.unmask();
	...
	function changetype(e){
		...
		if(e.value=="2"){//事假
		nui.alert("提醒：调休请暂邮件审批，事假为当日没有工资的假期类型。");
