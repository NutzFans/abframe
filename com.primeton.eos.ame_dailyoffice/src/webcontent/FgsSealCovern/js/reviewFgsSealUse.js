...
function init(){
	...
	nui.ajax({
			...
			success: function (o){
				...
				//只有在发起环节才能进行附件和表单的修改
				if(o.workItem.activityDefID != "fgsyzsy_fqlc"){
	            ...
	            if(o.fgsyzgl.backList){
	           		nui.get("backTo").setData(o.fgsyzgl.backList);
	           		}
				},
			error: function(){}
		});
...
function SaveData() {
    if(form.validate()){  
    	...
    	var json = {fgsyzsy:o.fgsyzgl,misOpinion:data_opioion.misOpinion,workItemId:workitemid};
        //form.loading("提交中，请稍候...");
        nui.ajax({
            url: "com.primeton.eos.ame_dailyoffice.fgsyzglHandle.modifyFgsSealUse.biz.ext",
            data: json,
            type: 'POST',
            cache: false,
            contentType: 'text/json',
