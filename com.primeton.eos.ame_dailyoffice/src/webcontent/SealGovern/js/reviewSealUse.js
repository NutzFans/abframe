...
function init(){
	...
	var json=nui.encode({workItemID: workitemid,misOpinion:opioion_data.misOpinion});
	nui.ajax({
		...
		//只有在发起环节才能进行附件和表单的修改
		document.getElementById("rentNum_tr").style.display="none";
		if(o.workItem.activityDefID != "yzsy_fqyzsylc"){
			document.getElementById("field1").disabled=true;
			$("#field2").hide();
			var grid = nui.get("grid_0");
			grid.hideColumn(5);
		}
		if(o.workItem.activityDefID  == "yzsy_xzbfzrsp" ){
			document.getElementById("field1").disabled=false;
			document.getElementById("rentNum_tr").style.display="";
			nui.get("dh").setReadOnly(true);
			nui.get("orgname").setReadOnly(true);
			nui.get("username").setReadOnly(true);
			nui.get("yzlx").setReadOnly(true);
			nui.get("yysyyjwjmc").setReadOnly(true);
			nui.get("remark").setReadOnly(true);
			nui.get("date").setReadOnly(true);
		}
    	...
    	if("yzsy_fqyzsylc" == o.workItem.activityDefID){
        ...
        if(o.yzgl.backList){
       		nui.get("backTo").setData(o.yzgl.backList);
   		}
	},
	error: function(){}
	});
...
function SaveData() {
    if(form.validate()){  
    	...
    	var json = {misOpinion:data_opioion.misOpinion,workItemId:workitemid,yzsy:o.yzgl};
        //form.loading("提交中，请稍候...");
        nui.ajax({
            url: "com.primeton.eos.ame_dailyoffice.yzglHandle.modifySealUse.biz.ext",
            data: json,
            type: 'POST',
            cache: false,
            contentType: 'text/json',
            success: function (text) {
				...
				if(e==1){
					if(returnJson.result == '1'){
						alert("流程提交成功！");
//	                   	var activityDefID = nui.get("activitydefid").getValue();
//	                   	if("yzsy_xzbfzrsp"==activityDefID){
