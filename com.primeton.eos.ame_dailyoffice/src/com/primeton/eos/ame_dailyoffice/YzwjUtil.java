...
/**
 * @author zhengquan
 * @date 2019-03-22 15:26:31
 *
 */
@Bizlet("")
public class YzwjUtil extends BusinessFormBaseUtil{
	...
	public DataObject createSealUseProcess(String ywlx,DataObject formData,DataObject MisOpinion,String isSave) throws Exception{
		...
		String wjsy = formData.getString("wjsy");
		if(wjsy.length() > 30){
			wjsy = wjsy.substring(0, 30) + "...";
		}
		BusinessParam businessParam = AmeConstants.YWLX_UTIL_MAP.get(ywlx);
		String processDefName = businessParam.getProcessDefName();
		String processInstName = username + "发起单号为" + dh +"【" + wjsy + "】" + businessParam.getProcessInstName();
		String processInstDesc = username + "发起单号为" + dh +"【" + wjsy + "】" + businessParam.getProcessInstDesc();
	...
	public Map<String, Object> handle(DataObject formData,DataObject MisOpinion) {
		...
		return super.handle(formData,"yzwjid");
	}
	...
	public Map<String, Object> modifySealUse(Long workItemID,DataObject misOpinion,DataObject formData){
		...
		try {
			misOpinion = super.modifyYzProcess(workItemID,misOpinion);
	 ...
	 * @description 根据数据实体查询印章外借数据
	 * @return
	 * @throws Exception
	 */
	@Bizlet("查询印章外借列表")
	public DataObject [] querySealDeliveryList(DataObject page,DataObject criteria) throws Exception {
		String cbName = "com.primeton.eos.ame_dailyoffice.dailyoffice.AmeYzwjProcess" + "|" + "yzwjrq";
	...
	public boolean deleteDatas(DataObject[] formDatas){
		return super.delete(formDatas,"yzwjid");
	}
