...
@Bizlet("预算主体 - (合计-月度) - 导出")
public class ExportZTFillingExcel {

	@Bizlet("导出")
	public String export(String budgetYear) throws Exception {
		Session dbSession = new Session(DataSourceHelper.getDataSource());
		TotalZTDatasUtil totalZTDatasUtil = new TotalZTDatasUtil();
		Map<String, List<Entity>> ztDataMap = totalZTDatasUtil.getTotalZTDatas(budgetYear);
		List<Entity> columnList = ztDataMap.get("column");
		...
		Sheet sheet = workbook.createSheet("合计-主体");

		// 设置单元格宽度
		this.setColumnWidths(sheet, columnList.size());
		...
		// 第一部分：财务报表
		List<Entity> cwbbList = ztDataMap.get("cwbb");
		...
		this.createHeaderRow(cwbbHeaderRow, "第一部分：财务报表", "合计", headerStyle, columnList);
		...
		List<String> cwbbFieldNameList = columnList.stream().map(entity -> entity.getStr("field")).collect(Collectors.toList());
		// 写入树形数据
		...
		// 第二部分：人员及人均情况
		List<Entity> ryjrjqkList = ztDataMap.get("ryjrjqk");
		// 对根节点按照serial_number排序
		if (ryjrjqkList != null) {
		...
		this.createHeaderRow(ryjrjqkHeaderRow, "第二部分：人员及人均情况", "平均实际", headerStyle, columnList);
		...
		List<String> ryjrjqkFieldNameList = columnList.stream().map(entity -> entity.getStr("field")).collect(Collectors.toList());
		// 写入树形数据
		...
		// 第三部分：劳动生产率
		List<Entity> ldsclList = ztDataMap.get("ldscl");
		// 对根节点按照serial_number排序
		if (ldsclList != null) {
		...
		this.createHeaderRow(ldsclHeaderRow, "第三部分：劳动生产率", "合计", headerStyle, columnList);
		...
		List<String> ldsclFieldNameList = columnList.stream().map(entity -> entity.getStr("field")).collect(Collectors.toList());
		// 写入树形数据
		...
		// 第四部分：人工成本-全口径
		List<Entity> rgcbqkjList = ztDataMap.get("rgcbqkj");
		// 对根节点按照serial_number排序
		if (rgcbqkjList != null) {
		...
		this.createHeaderRow(rgcbqkjHeaderRow, "第四部分：人工成本-全口径", "合计", headerStyle, columnList);
		...
		List<String> rgcbqkjFieldNameList = columnList.stream().map(entity -> entity.getStr("field")).collect(Collectors.toList());
		// 写入树形数据
	...
	// 新增：设置所有列的宽度
	private void setColumnWidths(Sheet sheet, int columnSize) {
	...
	// 创建表头行
	private void createHeaderRow(Row headerRow, String twoColumnName, String threeColumnName, CellStyle headerStyle, List<Entity> columnList) {
		for (int i = 0; i < columnList.size(); i++) {
			...
			if (i == 0) {
				...
				cell.setCellValue(columnList.get(i).getStr("header"));
			}
