...
public class ExportMonthFillingExcel {

	@Bizlet("导出")
	public String export(String budgetYear) throws Exception {
		Session dbSession = new Session(DataSourceHelper.getDataSource());
		TotalMonthDatasUtil totalMonthDatasUtil = new TotalMonthDatasUtil();
		Map<String, List<Entity>> monthDataMap = totalMonthDatasUtil.getTotalMonthDatas(budgetYear);
		...
		Sheet sheet = workbook.createSheet("合计-月度");
		...
		// 第一部分：财务报表
		List<Entity> cwbbList = monthDataMap.get("cwbb");
		...
		// 第二部分：人员及人均情况
		List<Entity> ryjrjqkList = monthDataMap.get("ryjrjqk");
		// 对根节点按照serial_number排序
		if (ryjrjqkList != null) {
		...
		// 第三部分：劳动生产率
		List<Entity> ldsclList = monthDataMap.get("ldscl");
		// 对根节点按照serial_number排序
		if (ldsclList != null) {
		...
		// 第四部分：人工成本-全口径
		List<Entity> rgcbqkjList = monthDataMap.get("rgcbqkj");
		// 对根节点按照serial_number排序
		if (rgcbqkjList != null) {
