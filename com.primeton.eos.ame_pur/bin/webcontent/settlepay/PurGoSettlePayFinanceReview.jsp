...
<body>
	...
	<div class="nui-fit">
		...
		<form id="form1" method="post">
				...
				<div style="width:800px;height:55px;margin-left:185px;float:left;">
					<table style="table-layout:fixed;">
						<tr>
							...
							<td align="right" style="width:210px;">结算费用总计：</td>
							<td id="sumtotal" style="width:87px;" align="right" ></td>
						</tr>
						<tr>
							<td colspan="3"></td>
							<td align="right" style="width:245px;">结算最终总金额：</td>
			...
			<fieldset id="field3" style="border:solid 1px #aaa;padding:3px;width: 1250px;">
				...
				<div style="width: 500px;height:25px;margin-left: 235px;float:left;">
					<table style="table-layout:fixed;">
						<tr>
	...
	<script type="text/javascript">
		...
		function SaveData(){
			...
			makeGoSettlePayFinanceReview();
		}
		
		//调用财务审核逻辑流
    	function makeGoSettlePayFinanceReview(){
    		var data = form.getData();
    		var opioionformData = opioionform.getData();
    		var backTo = nui.get("backTo").getValue();
    		data.backTo = backTo;
    		data.expReiList.projectname = nui.get("projectno2").getText();
    		data.expReiList.custname = nui.get("custno2").getText();
    		//审核信息
    		...
    		var json = nui.encode(data);
    		nui.ajax({
    			url:"com.primeton.eos.ame_pur.settlePay.makeGoSettlePayFinanceReview.biz.ext",
    			data:json,
    			type:"post",
    			contentType:"text/json",
    	...
    	function sumtaxmon(){
    		...
    		if(tax){
    			if(!isNaN(tax)){
    				...
    				tax = tax/100;
    				//含税金额
		    		var purSettleSetamount = nui.get("purSettleSetamount").getValue();
		    		var notaxmon = purSettleSetamount / (1 + parseFloat(tax));
