...
<!-- 
  - Author(s): zhuyl
  - Date: 2016-12-27 17:27:59
  - Description:
-->
<head>
	<title>向我汇报的项目外包人员-项目经理</title>
...
<body>
	...
	<div class="nui-fix">
		<div class="mini-panel" title="向我汇报的项目外包人员" width="auto">
				...
				<div class="nui-toolbar" style="border-bottom:0;padding:0px;">
					<table style="width:100%;">
						...
						<tr>
			            	...
			            	<td align="left">
			            		<!-- 汇报对象 -->
			            		<input name="criteria._or[0]._expr[0]._value" class="nui-hidden" style="width:100px;" />
							    <input class="nui-hidden" name="criteria._or[0]._expr[0]._property" value="manager"/>
							    <!-- 项目经理 -->
							    <input name="criteria._or[0]._expr[1]._value" class="nui-hidden" style="width:100px;" />
							    <input class="nui-hidden" name="criteria._or[0]._expr[1]._property" value="projectmanager"/>
							</td>
			            </tr>
						<tr>
			...
			<div style="width:100%;">
				<div class="nui-toolbar" style="border-bottom:0;padding:0px;">
		            <table style="width:100%;">
		            	<tr>
		                    <td style="width:20%;">
		                        <a class="nui-button" id="changeBtn" iconCls="icon-asyicon" onclick="editChange()">外包人员信息变更</a>
		                        <a class="nui-button" id="changeProjectBtn" iconCls="icon-edit" onclick="editProjectChange()">外包人员项目变更</a>
		        ...
		        <div id="datagrid1" class="nui-datagrid" style="width: 100%;height:325px;" allowResize="true" dataField="projectOutpers" 
			    	url="com.primeton.eos.ame_pur.outperson.queryReportingToManager.biz.ext" sortMode="client" 
	...
	<script type="text/javascript">
    	...
    	function init(){
    		//汇报对象
		    nui.getbyName("criteria._or[0]._expr[0]._value").setValue("<%=userno %>");
		    nui.getbyName("criteria._or[0]._expr[0]._value").setEnabled(false);
		    //项目经理
		    nui.getbyName("criteria._or[0]._expr[1]._value").setValue("<%=userno %>");
		    nui.getbyName("criteria._or[0]._expr[1]._value").setEnabled(false);
		    //服务状态缺省去掉已离场
		...
		function reset(){
			form.reset();
			//汇报对象
			nui.getbyName("criteria._or[0]._expr[0]._value").setValue("<%=username %>");
			//项目经理
		    nui.getbyName("criteria._or[0]._expr[1]._value").setValue("<%=userno %>");
			//服务状态缺省去掉已离场
		    nui.getbyName("criteria._expr[5]._value").setValue("1,2,3,4");
        ...
        function editChange(){
    		...
    		if (row) {
	      		nui.open({
		            url: "<%=request.getContextPath() %>/ame_pur/outperson/PurOutpersonChange.jsp",
		            title: "外包人员信息变更",
		            width: 1000,
		            height: 450,
		            onload: function () {
						var iframe = this.getIFrameEl();
						var data = {action: "editChange", outperno: row.outperno };
         ...
         *	外包人员项目变更
         */
        function editProjectChange(){
        	var row = grid.getSelected();
    		if (row) {
	      		nui.open({
		            url: "<%=request.getContextPath() %>/ame_pur/outperson/PurOutpersonProjectChange.jsp",
		            title: "外包人员项目变更",
		            width: 1000,
		            height: 450,
		            onload: function () {
						var iframe = this.getIFrameEl();
						var data = {action: "editProjectChange", outperno: row.outperno };
						iframe.contentWindow.SetData(data);
					},
					ondestroy: function (action) {
						if(action=="ok"){
        ...
        function selectionChanged(){
        	...
        	if(rows.length>1){ 
                ...
                nui.get("changeProjectBtn").disable();
            }else if(rows.length == 1){
            	if(rows[0].currentstatus == "4"){	//报备审核中
            		nui.get("changeBtn").disable();
                	nui.get("changeProjectBtn").disable();
            	}else{
	                nui.get("changeBtn").enable();
    	            nui.get("changeProjectBtn").enable();
            	}
            }else{
            	nui.get("changeBtn").enable();
	            nui.get("changeProjectBtn").enable();
