				...
				<div style="padding:5px;">
					<table style="table-layout:fixed;" id="table_file">
						<tr>
							...
							<td colspan="3"><input name="purSettle.custid"  onbuttonclick="changeCust" readOnly="readOnly" <%--onvaluechanged="changeCust"--%> id="custid"  valueField="custid" textField="custname" class="nui-buttonedit" emptyText="请选择供应商" required="true" style="width:200px" allowInput="false" />
							</td>
						</tr>
				...
				<div style="padding:5px;">
					...
					<table style="table-layout:fixed;" id="table_file">
						<tr>
							...
							<td><input name="expReiList.expRei.total" id="setamount2"  inputStyle="text-align: right;" onvaluechanged="changeSetamount"  class="nui-textbox" required="true" style="width:220px" />&nbsp  (元)</td>
						...
						<tr>
							...
							<td><input name="expReiList.expRei.companyname" id="companyname"  class="nui-textbox" required="true" style="width:150px"/></td>
						</tr>
						<tr style="display:none">
						...
						<tr>
		                    ...
		                    <td colspan="5"><input name="checkid" id="checkid" class="nui-buttonedit" required="true" style="width:240px;" allowinput="false" onbuttonclick="selectSaleLeader"/>
					...
					<table style="width:100%;" id="tictable">
						...
						<tr>
							...
							<td align="left" style="width: 19%;">    
								<input name="ticket.ticnum" style="width:150px" id="ticnum" class="nui-textbox" required="true"  onvaluechanged="checkTicketnum"/>
							</td>
							...
							<td align="left" >
								<input class="nui-datepicker" style="width:150px" name="ticket.ticdate" id="ticdate" required="true" >
							</td>
						</tr>
						<tr>
							<td align="right" style="width:120px;">发票金额：</td>
							<td align="left" style="width:300px;">    
								<input name="ticket.ticmon" style="width:150px"  id="ticmon" class="nui-spinner" inputStyle="text-align: right;"  format="n2"  minValue="0" required="true" showButton="false" onvaluechanged="countMonnet" allowLimitValue="false"/>
							</td>
							...
							<td align="left" >    
								<input name="ticket.ticmonnet" style="width:150px" id="ticmonnet" class="nui-spinner" inputStyle="text-align: right;"  format="n2"  readonly="readonly" showButton="false" allowLimitValue="false"/>
							</td>
						</tr>
						<tr>
...
<div style="text-align:center;padding:10px;" class="nui-toolbar">
   ...
   <a class="nui-button" onclick="onOk('app')" style="width:60px;margin-right:20px;" id="appButton">提交</a>
   <a class="nui-button" onclick="onOk('stop')" style="width:80px;margin-right:20px;" id="saveButton">终止流程</a>
   ...
   <input type="hidden" name="fileName" filter="false"/>
	...
	<script type="text/javascript">
		...
		function init(){
			...
			promptInit({"workItemID":<%=workItemID %>,"processDefName":"com.primeton.eos.ame_pur.ame_purOtherSettlePay","activityDefID":"manualActivity"});
			...
			nui.ajax({
				...
				success:function (o){
					...
					if(ticket!=null){
						...
						document.getElementById("field23").style.display="none";
					}
					...
					//nui.get("custno2").setValue(o.purSettle.miscustid);//客户id
					...
					nui.get("tickcustid").setValue(o.purSettles[0].custid);
					nui.get("tickpurcontid").setValue(o.purSettles[0].purcontract.purcontid);
					nui.get("tickstate").setValue("1");
			        
					var custid = o.purSettles[0].custid;
					var purcontid = o.purSettles[0].purcontract.purcontid;
                	...
                	grid2.load(jsonPresettle,function(){
                		...
                		for(var i = 0;i < rowsPresettle.length;i++){
                			if(rowsPresettle[i].settlementid != o.purSettles[0].settlementid){
		...
		function checkticket(){
			...
			if(row!=null){
				nui.get("tickcustid").setValue(row.purSupplier.custid);
				nui.get("tickpurcontid").setValue(row.purcontid);
				nui.get("tickstate").setValue("1");
			}
		}
		...
		function onSelectCont(e){ 
            ...
            if(rows.purcontid == null || rows.purcontid == 0){
                ...
                var json_t = nui.decode({"criteria": {"_expr[0]": {"_property": "purcontid","_op": "=","_value": rows.purcontid},"_expr[1]": {"_property": "state","_op": "=","_value": "1"}},"b": "true"});
		...
		function SaveData(){
		   ...
		   var rows = grid.getSelecteds();
			...
			if(form.validate()){
			     ...
			     //暂时保存
			     if(save == "stop"){
			          nui.confirm("结算最终总金额为" + setamount + "(元),"+massage+"确认暂时终止流程？", "确定？",
			          function (action) {
			              if (action == "ok"){
					          ...
					          for(var i=0;i<rdProject.length;i++){
					         		alert(rdProject[i].settlementno)
							  ...
							  if(purPresettles.length<1){
							  	data.purSettle.costtype="0";
							  }else{
							  	data.purSettle.costtype=purPresettles[0].costtype;
							  }
							  
							  //纸质凭证送递人信息
							  var checkerid = nui.get("checkid").getValue();
							  var checkername = nui.get("checkid").getText();
							  ...
							  nui.ajax({
					    			...
					    			success:function(action){
					    				...
					    				if(action.exception == null){
					    					...
					    					nui.alert("终止失败，请联系信息技术部人员！","系统提示", function(action){
