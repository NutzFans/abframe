...
<body>
<!-- 让IE8/9支持媒体查询，从而兼容栅格 -->
<div style="margin: 0 auto; width: 97%;height: auto;">
  <form class="layui-form layui-form-pane" lay-filter="dataFrm" id="dataFrm" >
    ...
    <i id="status" class="layui-icon" style="font-size: 15px;float:right; color: #5FB878;">审批通过</i> 
	  ...
	  <div class="layui-row">
	    <div class="layui-col-xs12">
		    <label class="layui-form-label">采购文件主要内容简述</label>
		    <div class="layui-input-block">
				...
				$.ajax({
			    	...
			    	var formData=data.purProgramme;
				    	...
				    	var gridInt = table.render({
						    ...
						    ,where: {"pid": formData.proappId} //如果无需传递额外参数，可不加该参数
								,cellMinWidth: 130
				  			,method: 'post' //如果无需自定义HTTP类型，可不加该参数
						    ,cols: [[
						      {field:'code', title: '计划编号'}
						      ,{field:'planName',  title: '计划名称'}
						      ,{field:'materialName',  title: '采购物项名称 '}
						      ,{field:'budgetAmount', title: '预算金额'}
						      ,{field:'amount', title: '立项金额'}  
						      ,{field:'needOrgName', title: '采购单位'}
						    ]]
...
// 						  });
		 ...
		 var fileGridInt =  table.render({
		    ...
		    ,where: {"groupid": "purProgramme","relationid":id,"sortField":"fileTime","sortOrder":"desc"}//如果无需传递额外参数，可不加该参数
  			,method: 'post' //如果无需自定义HTTP类型，可不加该参数
		    ,cols: [[
		      {field:'fileName',width:"80%", title: '附件名称' ,templet: "<div>{{getdetail(d)}}</div>"}
		      ,{field:'fileSize', width:"20%", title: '文件大小',templet: "<div>{{getFileSize(d.fileSize)}}</div>"}
		    ]]
		    ,parseData: function(res){ //res 即为原始返回的数据
			    return {
			      "code": "0", //解析接口状态
