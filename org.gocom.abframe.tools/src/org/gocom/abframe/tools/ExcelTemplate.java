...
/**
 *
 * Excel模板实现类，实现通过自定义Excel数据模版,将结果集填充到模版相应位置，自动创建输出到指定的文件，允许Excel模版设置公式
 */
public class ExcelTemplate {
	...
	/**
	 * 初始化工作模版，获取模版配置起始行(start)以及对应字段填充位置(fieldNames) 用于初始化简历
	 */
	...
	private void generateContentDatas(List<DataObject> resultset, HSSFWorkbook wb, HSSFSheet sheet) {
		HSSFCellStyle borderStyle = this.getBorderStyle(wb);
		HSSFCellStyle noneStyle = this.getNoneStyle(wb);
		HSSFCellStyle alignCenter = this.getBorderStyleAlign(wb, CellStyle.ALIGN_CENTER);
		...
		for (Iterator it = resultset.iterator(); it.hasNext(); autoRowId++) {
			...
			for (int i = 0; i < sourceRow.getPhysicalNumberOfCells(); i++) {
				...
				if (fieldNames[i] != null) {
					...
					if (content != null) {
						...
						if (value != null) {
							...
							if (this.templateFile.contains("purOutpersonsTemplate")) {
								if (fieldNames[i].equals("outpername") || fieldNames[i].equals("gender") || fieldNames[i].equals("degree") || fieldNames[i].equals("gradudate")
										|| fieldNames[i].equals("currentstatus") || fieldNames[i].equals("startdate") || fieldNames[i].equals("expenddate") || fieldNames[i].equals("actenddate")) {
								}
							}
					...
					if (sourceCell != null && sourceCell.getCellType() == Cell.CELL_TYPE_STRING && sourceCell.getRichStringCellValue().getString() != null
	...
	/**
	 * 将指定的对象数组resulset输出到指定的Excel位置 用于导出简历
	 */
	private void generateContentDatas4Resume(DataObject resume, HSSFWorkbook wb, HSSFSheet sheet) {
		...
		borderStyle1.setAlignment(CellStyle.ALIGN_LEFT);
		borderStyle1.setVerticalAlignment(CellStyle.VERTICAL_CENTER);
		borderStyle1.setWrapText(true);
		newStyle.setVerticalAlignment(CellStyle.VERTICAL_CENTER);
		newStyle.setWrapText(true);
		newStyle1.setVerticalAlignment(CellStyle.VERTICAL_TOP);
	...
	/**
	 * 将结果集填充到Excel模版,resultset必须是以Map封装行 用于导出简历,支持多sheet页
	 */
	...
	/**
	 * 将结果集填充到Excel模版,resultset必须是以Map封装行 支持多sheet页导出
	 */
	...
	/**
	 * 将结果集填充到Excel模版,resultset必须是以Map封装行 支持多sheet页动态导出(按照第一页的模版)
	 */
	 ...
	 * 多sheet页导出
	 */
	public void generateWithMoreSheet(Map<String, List<DataObject>> excelMap, DataObject[] exportInfo, String[] sheetNames) throws Exception {
		POIFSFileSystem fs = new POIFSFileSystem(new FileInputStream(templateFile));
		HSSFWorkbook wb = new HSSFWorkbook(fs);
		...
		for (int i = 0; i < sheetNames.length; i++) {
			...
			this.generateTitleDatas(exportInfo[i], wb, newSheet);
			// 生成数据内容
			this.generateContentDatas(excelMap.get(sheetNames[i]), wb, newSheet);
			if (hasFormula) {
				this.calcFormula(wb, newSheet);
			}
		}
	...
	/**
	 * 复制EXCEL行到指定的工作薄上 ××如果是分页显示，需要增加一个判断：当复制行包含公式forumla=sum(G7:G~)字样时候，必须修改其实行G7为相应的新行。
	 */
	...
	public Map importData4Resume(String targetFile, String resumeEntityName, String workExpEntityName, String projExpEntityName, String certEntityName) throws Exception {
		...
		for (int i = 1; i < 6; i++) {
			...
			} else if (cell1.getCellType() == Cell.CELL_TYPE_BOOLEAN) {
		...
		if (otherDescripCell.getCellType() == Cell.CELL_TYPE_NUMERIC) {
			if (DateUtil.isCellDateFormatted(otherDescripCell)) {
				SimpleDateFormat dateFormat = new SimpleDateFormat("yyyy-MM-dd HH:mm:ss");
		...
		} else if (otherDescripCell.getCellType() == Cell.CELL_TYPE_BOOLEAN) {
		...
		if (abilityCell.getCellType() == Cell.CELL_TYPE_NUMERIC) {
			if (DateUtil.isCellDateFormatted(abilityCell)) {
				SimpleDateFormat dateFormat = new SimpleDateFormat("yyyy-MM-dd HH:mm:ss");
				abilityVal = dateFormat.format((abilityCell.getDateCellValue()));
			} else {
				abilityVal = String.valueOf((long) abilityCell.getNumericCellValue());
			}
		} else if (abilityCell.getCellType() == Cell.CELL_TYPE_BOOLEAN) {
	 ...
	 * 将目标Excel文件的内容导入到数据表
	 */
	public DataObject[] importDataEmp(String excelFile, String EmpEntityName, String EduEntityName, String ConEntityName, String VerEntityName, String AcEntityName, String LevelEntityName)
			throws Exception {
