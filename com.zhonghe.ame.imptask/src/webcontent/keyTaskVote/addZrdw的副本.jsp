...
<body>
	<div class="nui-fit">
        ...
        <fieldset style="border: solid 1px #aaa;padding: 3px;width: 99%;" align="center">
		  <legend>被投票单位</legend>	
			<div class="nui-toolbar" style="border-bottom: 0;padding: 0px;" id="button_inorgid1" align="center">
	    	...
	    	<div id="grid_traveldetail" class="nui-datagrid" style="width: 100%;height: auto;" allowCellSelect="true" 
	    		showPager="false" allowCellEdit="true" multiSelect="true" align="center">
	            <div property="columns">
	            <div name="temp123" type="checkcolumn"></div>
	                <div field="orgid" displayField="orgname" width="100%" align="center" headerAlign="center">责任单位名称
			...
			<div class="nui-toolbar" style="border-bottom: 0;padding: 0px;" id="button_inorgid1" align="center">
	            <table style="width: 100%;" align="center">
	            	<tr>
	                    <td style="width: 20%;" align="center">
	                        <a class="nui-button" id="addbtn" iconCls="icon-add" onclick="addTicket1()">增加</a>
	                        <a class="nui-button" id="deltbtn" iconCls="icon-remove" onclick="removeTicket1()">删除</a>
	                    </td>
	                </tr>
	             </table>           
	        </div>
	    	<div id="grid_traveldetail1" class="nui-datagrid" style="width: 100%;height: auto;" allowCellSelect="true" 
	            ...
	            <div property="columns">
	            <div name="temp123" type="checkcolumn"></div>
	                <div field="id" displayField="orgname" width="100%" align="center" headerAlign="center">责任单位名称
	                <input property="editor" class="nui-spinner"  format="n2" allowLimitValue="false"/>
	                </div>
	            </div>
	        </div>
    ...
    <script type="text/javascript">
    	...
    	var grid_traveldetail1 = nui.get("grid_traveldetail1");
		...
		function removeTicket1(){
		var rows = grid_traveldetail1.getSelecteds();
        if (rows.length>0) {
            grid_traveldetail1.removeRows(rows, true);
        }else{
        	nui.alert("请至少选中一条记录！");
        }
	}
             ...
             if(action == "ok"){
                 ...
                 if(length>0){
                     for(var i=0;i<data.length;i++){
		                 var prodmod="";
			                 if(data[i].moduleid){
			                 	prodmod = data[i].prodname+"--"+data[i].modulename;
			                 }else{
			                 	prodmod = data[i].prodname;
		                	 }
					         var newRow = { name: "New Row",id:data[i].orgid,orgname:data[i].orgname};
					         grid_traveldetail.addRow(newRow);
                   			  }
                			 }
           			  }
       			 }
    	...
    	function addTicket1(){
	    	nui.open({
        	url: url+"/imptask/orgList.jsp",
        	title: "新增信息", width: 600, height: 300,
        	onload: function () {
        	},
        	ondestroy: function (action) {
             if(action == "ok"){
                 ...
                 if(length>0){
                     for(var i=0;i<data.length;i++){
		                 var prodmod="";
					         ...
					         var newRow = { name: "New Row",id:data[i].orgid,orgname:data[i].orgname};
					         grid_traveldetail1.addRow(newRow);
                   			  }
                			 }
           			  }
       			 }
	...
	function submit(){
    			...
    			for(j = 0,len=arr.length; j < len; j++) {
    				if(arr[j].id!=undefined){
    					...
    					nui.alert("参与投票单位不能为空");
    				}
				}
				
				
				var arr1=grid_traveldetail1.data;
    			
    			var orgids = "";
    			for(j = 0,len=arr1.length; j < len; j++) {
    				if(arr1[j].id!=undefined){
    					if(orgids==""){
    					orgids = arr1[j].id;
    					}else{
    					orgids = orgids+","+arr1[j].id;
    					}
    				}else{
    					nui.alert("参与投票单位不能为空");
    				}
    			...
    			var json = nui.encode({"taskVote":data,"ids":ids,"orgids":orgids}); 
