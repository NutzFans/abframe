			...
			for (int i = 0; i < itemDatas.length; i++) {
				...
				if ("added".equals(entity.getStr("_state"))) {
					Entity insert = Entity.create("zh_key_task_group_item").set("id", IdUtil.objectId()).set("main_id", mainId).set("task_month", entity.getInt("taskMonth"))
							...
							.set("responsible_person", entity.getStr("responsiblePerson"));
					dbSession.insert(insert);
				} else if ("modified".equals(entity.getStr("_state"))) {
					String updateSql = "UPDATE zh_key_task_group_item SET task_month = ?, task_plan_name = ?, create_user = ?, create_name = ?, create_time = ?, responsible_person = ? WHERE id = ?";
					dbSession.execute(updateSql, entity.getInt("taskMonth"), entity.getStr("taskPlanName"), createUser, createName, DateTime.now(), entity.getStr("responsiblePerson"),
							entity.getStr("id"));
