	...
	<div class="nui-fit" style="padding: 5px;">
		<form id="editForm" method="post">
			<fieldset id="field1" style="border: solid 1px #aaa;">
				<legend>定时任务 - 编辑</legend>
				<div style="padding: 5px;">
					<table style="table-layout: fixed;">
						<tr>
							<td align="right" style="width: 100px">任务名称：</td>
							<td>
								<input id="taskName" name="taskName" class="nui-textbox" style="width: 200px" enabled="false" />
							</td>
		...
		function setCronExpression(cronExpression) {
			...
			if (times.length == 6) {
				if ((day == "*" || day == "?") && (month == "*" || month == "?") && (week == "*" || week == "?")) {
					...
					triggerOuterLayerRadioChange({
						"value" : "day"
					});
				} else if (day != "*" && day != "?") {
					if ((month == "*" || month == "?") && (week == "*" || week == "?")) {
						nui.get("radioMonth").setValue("month");
						triggerOuterLayerRadioChange({
							"value" : "month"
						});
						triggerInnerLayerRadioChange("radioMonthDay");
						nui.get("selectMonthDay").setValue(day);
					} else if (month != "*" && month != "?" && (week == "*" || week == "?")) {
						nui.get("radioYear").setValue("year");
						triggerOuterLayerRadioChange({
							"value" : "year"
						});
						triggerInnerLayerRadioChange("radioYearMonthDay");
						nui.get("selectYearMonth").setValue(month);
						nui.get("selectYearMonthDay").setValue(day);
					}
				} else if (month != "*" && month != "?") {
					nui.get("radioYear").setValue("year");
					triggerOuterLayerRadioChange({
					...
					triggerInnerLayerRadioChange("radioYearWeek");
					if (week.indexOf("#") != -1) {
						var tw = week.substring(0, 1);
						var tm = week.substring(2, 3);
						nui.get("selectYearMonthOrder").setValue(month);
						nui.get("selectYearWeekOrder").setValue(tm);
						nui.get("selectYearWeek").setValue(tw);
					} else if (week.indexOf("L") != -1) {
						var tw = week.substring(0, 1);
						nui.get("selectYearMonthOrder").setValue(month);
						nui.get("selectYearWeekOrder").setValue("-1");
						nui.get("selectYearWeek").setValue(tw);
					}
				} else if (week != "*" && week != "?") {
					if ((day == "*" || day == "?") && (month == "*" || month == "?")) {
						if (week.indexOf(",") != -1) {
							nui.get("radioWeek").setValue("week");
							triggerOuterLayerRadioChange({
								"value" : "week"
							});
							nui.get("selectWeek").setValue(week);
							...
							triggerInnerLayerRadioChange("radioMonthWeek");
							nui.get("selectMonthWeekOrder").setValue(tm);
							nui.get("selectMonthWeek").setValue(tw);
						} else if (week.indexOf("L") != -1) {
							var tw = week.substring(0, 1);
							nui.get("radioMonth").setValue("month");
							triggerOuterLayerRadioChange({
							...
							triggerInnerLayerRadioChange("radioMonthWeek");
							nui.get("selectMonthWeekOrder").setValue("-1");
							nui.get("selectMonthWeek").setValue(tw);
						} else {
							nui.get("radioWeek").setValue("week");
							triggerOuterLayerRadioChange({
				...
				if (year == "*" || year == "?") {
					if ((day == "*" || day == "?") && (month == "*" || month == "?") && (week == "*" || week == "?")) {
						...
						if (week.indexOf("#") != -1) {
							...
							var tw = week.substring(0, 1);
							nui.get("selectYearWeekOrder").setValue(month);
							nui.get("selectYearWeekOrder").setValue("-1");
							nui.get("selectYearWeek").setValue(tw);
						}
					} else if (week != "*" && week != "?") {
				...
				nui.confirm("确定保存数据吗？", "系统提示", function(action) {
					if (action == "ok") {
						...
						setTimeout(function() {
							...
							ajaxCommon({
								"url" : "com.zhonghe.ame.scheduler.task.scheduledTasks.updateScheduledTasks.biz.ext",
								data : json,
