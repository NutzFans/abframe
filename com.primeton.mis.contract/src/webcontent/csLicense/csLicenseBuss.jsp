...
<body>
	<div class="nui-fit">
		...
		<jsp:include page="/contract/csLicense/emailForm.jsp"/>
...
<script type="text/javascript">
		...
		promptInit({workItemID: <%=workItemID %>});//流程提示信息
  		...
  		nui.ajax({//获取许可申请信息
      		...
      		success: function (o){
  				...
  				var inceptperson = csLiceSend.inceptperson;//接收人
            	...
            	document.getElementById("contreg").innerHTML = cscontract.contreg == null ?"":cscontract.contreg;
  				...
  				nui.get("inceptperson").setText(inceptperson);
				...
				if("manualActivity2" == activityDefID){
					...
					document.getElementById("email").style.display="";
  					...
  					nui.get("senddate").setRequired(true);
  					nui.get("senddate").setValue(new Date());
  					
  					var inceptpersonmail = csLiceSend.inceptpersonmail;
  					
  					if(inceptperson != null && inceptperson !="" && inceptpersonmail != null && inceptpersonmail !=""){
	  					var inceptpersonText = inceptperson+"< "+inceptpersonmail+">";
			       		nui.get("mailtolist").setValue(inceptpersonmail);
			       		nui.get("mailtolist").setText(inceptpersonText);
	  				}
	  				var sales = csLiceSend.sales;
	  				var cclistValue="";
	  				var cclistText="";
	  				if(sales){
	  					for(var i = 0 ;i < sales.length ; i++){
	  						if(cclistValue !=""){
	  							cclistValue = cclistValue+","+sales[i].oemail;
		  						cclistText = cclistText+","+sales[i].salename+"< "+sales[i].oemail+">";
	  						}else{
	  							cclistValue = sales[i].oemail;
	  							cclistText = sales[i].salename +"< "+sales[i].oemail+">";
	  						}
	  					}
	  				}
	  				var othermail = selectContractType();
	  				var othermailValue = othermail.dictname;
	  				var othermailText = othermail.filter1;
	  				
	  				if(othermailValue != null && othermailValue != "" && othermailText != null && othermailText != ""){
	  					var othermailValues = othermailValue.split(",");
	  					var othermailTexts = othermailText.split(",");
	  					
	  					for(var i = 0 ;i < othermailValues.length ; i++){
	  						if(cclistValue !=""){
	  							cclistValue = cclistValue+","+othermailValues[i];
		  						cclistText = cclistText+","+othermailTexts[i]+"< "+othermailValues[i]+">";
	  						}else{
	  							cclistValue = othermailValues[i];
	  							cclistText = othermailTexts[i]+"< "+othermailValues[i]+">";
	  						}
	  					}
	  					
	  				}
	  				if(cclistValue !=""){
  						cclistValue = cclistValue+","+nowusermail;
	  					cclistText = cclistText+","+nowusername+"< "+nowusermail+">";
  					}else{
  						cclistValue = nowusermail;
  						cclistText = nowusername+"< "+nowusermail+">";
  					}
	  				if(cclistValue !=""){
			       		nui.get("mailcclist").setValue(cclistValue);
			       		nui.get("mailcclist").setText(cclistText);
	  				}
	  				nui.get("subject").setValue(csLiceSend.mail.subject);
	  				editor2.html(csLiceSend.mail.message);
    ...
    function onOk(e){
    	...
    	nui.confirm(msg, "操作提示",function (action) {            
            if (action == "ok") {
            	document.getElementById("fileCatalog2").value="csLiceSend";
				form6.submit();
            }
        });
    }
	function SaveData2(){
		...
		var csLicProds = csLicProd_grid.getData();
    	...
    	if("manualActivity3" == activityDefID){
    		...
    		var json =nui.encode({csLiceSend:om.csLiceSend,misOpinion:misOpinion,csLicProds:csLicProds,email:email1.mailinfo});
    	}else{
	...
	function selectContractType(){
		var json = nui.encode({"dictid":"csLicenseMail","dicttypeid":"MAIL_RECIPIENT"});
		var entry;
    	nui.ajax({
