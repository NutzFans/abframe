...
<title>编辑竞争对手</title>
		...
		<form id="form1" method="post">
			<input name="competId" id="competId" class="nui-hidden" />
	...
	<script type="text/javascript">
		...
		function setEditData(data) {
			nui.get("competId").setValue(data.competId);
		...
		function onOk() {
			...
			var json = nui.encode({'competInfo' : data});
			if (!form.validate()) {
				showTips("请检查表单的完整性!", "danger");
				return;
			}
			ajaxCommon({
				"url" : "com.zhonghe.ame.marketInfo.marketinfo.khxx.bid.editCompetInfo.biz.ext",
				"data" : json,
				"success" : function(data) {
					if(data.result=="1"){
						...
						showTips("系统中已存在同名竞争对手，无法编辑保存！", "danger");
					}else{
						showTips("编辑竞争对手失败，请联系管理员！", "danger");
					}
				}
			});
		}		
