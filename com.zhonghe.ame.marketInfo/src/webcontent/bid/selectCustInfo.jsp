...
<body>
	...
	<script type="text/javascript">
		...
		function add() {
			nui.open({
				url : "/default/marketInfo/bid/addCustInfo.jsp",
				width : '580',
				height : '150',
				title : "新增客户",
				onload : function() {
		...
		function edit() {
			...
			if (row.length > 1 || row.length == 0) {
				...
				var data = row[0];
				nui.open({
					url : "/default/marketInfo/bid/editCustInfo.jsp",
					width : '580',
					height : '150',
					title : "编辑客户",
					onload : function() {
		...
		function deleteInfo() {
			...
			if (row.length > 1 || row.length == 0) {
				...
				if (!confirm("是否删除？")) {
					...
					nui.ajax({
						url : "com.zhonghe.ame.marketInfo.marketinfo.khxx.bid.deleteCustInfo.biz.ext",
						type : 'POST',
						data : json,
