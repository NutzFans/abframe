...
<div>
	<fieldset id="field2" style="border:solid 1px #aaa;padding:3px;">
			<form id="form6" class="" action="com.primeton.eos.ame_common.ame_fileupload2.flow" target="hidden_frame2" method="post" enctype="multipart/form-data">
				<!--附件存放目录-->
				<input type="hidden" name="atFileupload/fileCatalog" id="fileCatalog2" value=""/>
				<table id="filetable2" width="100%" style="border-collapse:collapse" bordercolor="#A8A8A8" border="1" cellspacing="0" cellpadding="1"  align="center">
					<tr><!-- 保持一致限制必须上传文件时，比如收入预估流程编辑页面要读取这个table的data.length -->
						<input type="button" value="增加附件" onclick="addFile2()"/>
	...
	<script>
        ...
        function addFile2(){
            z++;
	        var myTable = document.getElementById('filetable2');
			//向表格中增加一行
			...
			oCell1_2.innerHTML="<input type='file'  name='uploadfile2' size='30' onchange='checkempty2(this.value," + tempID2 + ")';/>";
			
			oCell1_3.innerHTML="<font style='color:#FF0000;'>*</font>附件名称";
			
			var temp = '<input type="text" name="remarkList2" disabled="true" id="'+tempID+'"  size="30" validateAttr="allowNull=false"  size="45"/>';
			oCell1_4.innerHTML= temp;		
			oCell1_5.innerHTML= "<input type='button' value='删除附件' onclick='delFile2(this);'/>";		
			return;
        }
         <!--//删除附件-->
		...
		function checkempty2(objvalue,flag){
