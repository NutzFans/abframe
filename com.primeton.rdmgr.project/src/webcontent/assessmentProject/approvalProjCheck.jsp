			...
			<input name="rdprojcheck.projectid" id="projectid" class="nui-hidden"/>
		     ...
		     <fieldset id="field41" style="border:solid 1px #aaa;padding:3px;">
		       <legend>项目考核分配</legend>
		       		<table style="table-layout:fixed;" id="a3">
		       			<tr id="a33">
		       				<td align="right" style="width:180px;">奖金发放年月：</td>
							<td style="width:150px" >
								<input id="costyearmonth" name="rdprojcheck.costyearmonth" allowInput="false" class="nui-monthpicker" inputStyle="text-align: right" style="width:150px" format="yyyy-MM" onvaluechanged="checkDate"/>
							</td>
		       			</tr>
		       			<tr>
					...
					<div class="nui-tabs" activeIndex="0">
						<div title="项目组内分配方案">
			     			<div id="projEmp" class="nui-datagrid" style="width:100%;height:auto;" allowCellEdit="true" allowCellSelect="true" editNextOnEnterKey="true"
							    dataField="projAllots" showSummaryRow="true" ondrawsummarycell="drawSum1" sortMode="client"
							    allowResize="true" allowAlternating="true" multiSelect="true" showPager="false" allowCellWrap="true">
							    <div property="columns">
							        <div field="empname" width="40" headerAlign="center" align="center" allowSort="true">人员姓名</div>
							        <div field="empstatus" width="40" align="center" headerAlign="center" renderer="getEmpstatus" allowSort="true">在职状态</div>
							        <div field="workdays" dataType="float" width="50" align="right" headerAlign="center" allowSort="true" summaryType="sum">人天数</div>
							        <div field="projrole" width="50" align="center" headerAlign="center" allowSort="true">项目职责</div>
							        <div field="evaluate" width="120" align="left"headerAlign="center" allowSort="true">考核评价</div>
							        <div field="num" width="40" align="right" headerAlign="center" allowSort="true">贡献度打分</div>
							        <div field="sum" dataType="float" width="40" align="right" headerAlign="center" summaryType="sum" numberFormat="n2" allowSort="true">奖金</div>
							        <div field="remark" width="120" align="left"headerAlign="center" allowSort="true">备注</div>
							    </div>
							</div>
			     		</div>
			     		<div title="考核评分">
			     			<div id="distribution" class="nui-datagrid" style="width:100%;height:auto;" dataField="scores" editNextOnEnterKey="true"
							    url="com.primeton.rdmgr.project.assessmentProject.queryRdScore.biz.ext" allowResize="true" showSummaryRow="true" ondrawsummarycell="drawSum11"
							    ...
							    <div property="columns">
							        ...
							        <div field="identitytype" width="50" headerAlign="center" align="center" renderer="getIdentity">身份</div>
...
<div style="text-align:center;padding:10px;" class="nui-toolbar">
		<a class="nui-button" onclick="exportCost" id="exportCost" style="width: 120px;margin-right:20px;">导出项目奖金分配</a>  
...
<form name="viewlist1" id="viewlist1" action="com.primeton.eos.ame_common.ameExportCommon.flow" method="post">
	...
	<script type="text/javascript">
		...
		function init(){
			...
			nui.ajax({
				...
				success: function (o){
            		...
            		if(workitem.activityDefID=='manualActivity6'){
            			document.getElementById("exportCost").style.display="";
						document.getElementById("a33").style.display="";
						nui.get("costyearmonth").setRequired(true);
            		}
            		if(projcheck.costyearmonth!=null&&projcheck.costyearmonth!=""){
            			var costyearmonth = projcheck.costyearmonth;
					  	costyearmonth = costyearmonth.substring(0,4)+"-"+ costyearmonth.substring(4,6);       			
            			nui.get("costyearmonth").setValue(costyearmonth);
            		}
            		var projAllots = o.projAllots;
            		var projEmp = nui.get("projEmp");
            		projEmp.setData(projAllots);
            		projEmp.sortBy("sum","desc");
					...
					if(projcheck.labordiffcost){
						document.getElementById("labordiffcost").innerHTML = "<a href='javascript:void(0)' onclick='checkDetail1();' title='点击查看'>"+ formatNumber(projcheck.labordiffcost) +"</a>";
					}
		...
		function SaveData(){
			...
			var costyearmonth1 = nui.get("costyearmonth").getValue();
			if(costyearmonth1!=null&&costyearmonth1!=""){
				rdprojcheck.costyearmonthdate = costyearmonth1;
				var costyearmonth = formatDateTime(costyearmonth1);
				rdprojcheck.costyearmonth = costyearmonth;
			}
			
			var json = nui.encode({"rdprojcheck":rdprojcheck,"misOpinion": data_opioion.misOpinion});
			 nui.ajax({
		...
		function drawSum1(e){
			if (e.field == "sum"||e.field == "workdays") {
				totalsum = e.cellHtml;
	            e.cellHtml = "<div align='right'><b>" + e.cellHtml + "</b></div>";
	        }
		}
		
		function drawSum11(e){
		...
		function exportCost(){
			...
			nui.ajax({
    			url: "com.primeton.rdmgr.project.assessmentProject.exportRdScore.biz.ext",
		        type: "post",
		        data: json,
		        cache: false,
		        contentType: 'text/json',
		...
		function checkDate(){
			var costyearmonth = nui.get("costyearmonth").getValue();
			var year1 =  costyearmonth.getFullYear();
			var month1 =  costyearmonth.getMonth()+1;
			var yearmonth1 = year1+month1;
			var date = new Date();
			var year = date.getFullYear(); //获取完整的年份(4位)
			var month = date.getMonth()+1; //获取当前月份(0-11,0代表1月)
			var yearmonth = year+month;
			if(yearmonth1<yearmonth){
				alert("发放日期必须大于等于当前年月！");
				nui.get("costyearmonth").setValue(null);
			}
		}
