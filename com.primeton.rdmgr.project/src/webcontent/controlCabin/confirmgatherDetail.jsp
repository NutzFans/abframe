...
<body>
<div class="table-responsive" id="gathercontain">
	<table class="table table-striped table-hover text-nowrap" id="gather">
		<thead>
			<tr>
				...
				<th data-field="gatherno" class="center"  data-align="center" data-halign = "center">收款编号</th>
				<th data-field="yqrje" class="center" data-align="right" data-halign = "center" data-formatter="formatnum" data-footer-Formatter="contsumall" data-width = "150px">收款金额</th>
				<th data-field="wqrje" class="center" data-align="right" data-halign = "center"  data-visible="false" data-formatter="formatnum" data-footer-Formatter="contsumall1" data-width = "150px">预计收款金额</th>
				<th data-field="maconfirmday" class="center" data-align="center" data-halign = "center"  data-formatter="TimeFormatter">收款日期</th>
				<th data-field="billstatus" class="center" data-align="center" data-visible="false" data-halign = "center" data-formatter="statusFormatter">开票状态</th>
		...
		<tfoot>
			<tr>
				...
				<th id='yqrje' style="text-align:right"></th>
				<th id='wqrje' hidden="hidden" style="text-align:right"></th>
				<th id='maconfirmday'></th>
				<th id='billstatus' hidden="hidden"></th>
				<th id='salename'></th>
			</tr>
		</tfoot>
	</table>
...
<script type="text/javascript">
 		...
 		$.ajax({
			...
			success:function(data){
		    	var datalist = data.objs;
		     	$('#gather').bootstrapTable('destroy').bootstrapTable({  //'destroy' 是必须要加的==作用是加载服务器//  //数据，初始化表格的内容Destroy the bootstrap table.
		     	...
		     	if(status == '0'){
		    		$('#gather').bootstrapTable('hideColumn', 'yqrje');
			    	document.getElementById("yqrje").hidden = 'hidden';
		    		$('#gather').bootstrapTable('hideColumn', 'maconfirmday');
			    	document.getElementById("maconfirmday").hidden = 'hidden';
		    		$('#gather').bootstrapTable('showColumn', 'wqrje');
			    	document.getElementById("wqrje").hidden = '';
		    		$('#gather').bootstrapTable('showColumn', 'billstatus');
			    	document.getElementById("billstatus").hidden = '';
		    	}
	...
	function statusFormatter(value,row,index){
		var json = {"dicttypeid":"MIS_GATHERBILL","dictid":value};
		var dictname ='';
		$.ajax({
		   	type:"POST",
		   	async:false,
	...
	function contsumall(rows){
        ...
        document.getElementById("yqrje").innerHTML = formatCurrency(sum,null,null);
	...
	function contsumall1(rows){
        ...
        document.getElementById("wqrje").innerHTML = formatCurrency(sum,null,null);
        return formatnum(sum,null,null);
	}
