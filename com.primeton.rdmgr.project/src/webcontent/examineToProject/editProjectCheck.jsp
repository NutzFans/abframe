...
<div style="text-align:center;padding:10px;" class="nui-toolbar">
   ...
   <a class="nui-button" onclick="onOk('1')" style="width:140px;margin-right:20px;" id="appButton">发起项目考核流程</a>
	...
	<script type="text/javascript">
		...
		function init(){
			...
			nui.ajax({
				...
				success: function (o){
					...
					nui.get("projectnames").setText(rdprojcheck.projectname);
                    nui.get("projectnames").setValue(rdprojcheck.projectno);
                    ...
                    nui.get("custid").setValue(rdprojcheck.custid);
                    //预算信息
                    var rds = o.rds;
                    if(rds){
                    	var data = rds;
                        ...
                        if(nui.encode(data[0].cbversion)!='null'){
                            ...
                            nui.get("budgetcost").setValue(parseFloat(budget-outbuy).toFixed(2));
                        }
                    }
                    if(rdprojcheck.assessmentcost){
                    ...
                    nui.get("checkincomeym").setValue(rdprojcheck.checkincomeym);
		...
		function SaveData(){
			...
			var json = nui.encode({"sybmanager":a,"technology":b,"project":project,"rdprojcheck":rdprojcheck,"misOpinion": data_opioion.misOpinion});
			 nui.ajax({
                url: "com.primeton.rdmgr.project.examineToProject.submitPojectCheck.biz.ext",
                data: json,
                type: 'POST',
                cache: false,
                contentType: 'text/json',
