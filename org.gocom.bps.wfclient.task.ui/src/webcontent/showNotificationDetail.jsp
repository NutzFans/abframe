...
<body style="height:450px;width: 97%;">
	<div id="workItemTabs" class="nui-tabs" style="margin:0 auto;width: 100%;height: 99%" activeIndex="-1">
		...
		<div id="workItemDetailTab" name="workItemDetailTab" title="流程通知基本信息">
			<form id="from1">
				<fieldset>
	...
	<div class="bottomBtnDiv">
		<a id="executeButton" class="nui-button redBtn" onclick="confirm">已阅</a>
		<a id="closeButton" class="nui-button redBtn" onclick="doCancel"><%=I18nUtil.getMessage(request, "bps.wfclient.common.Closed")%></a>
	</div>
	<script type="text/javascript">
		...
		function initData(row) {
    		processGraphObj.setProcInstID(row.procInstID);
			...
			}else if(row.state=="UNVIEWED"){
				nui.get("closeButton").setVisible(false);
    		...
    		nui.ajax({
				...
				type:"post",
				data:{processinstid: row.procInstID},
				contentType:"text/json",
				success:function (o){
					var url=o.PROCURL;
					if(url != null && url != ""){
			    		tab.url = "<%=request.getContextPath() %>"+url+"?processInstID=" + row.procInstID;
		    		}else{
			    		tab.url = "<%=request.getContextPath() %>/bps/wfclient/task/noYWpage.jsp";
		    		}
		    		tabsObj.addTab(tab,0);
		    		tabsObj.loadTab(tab.url,tab);
		    		tabsObj.activeTab(tab);
				},
