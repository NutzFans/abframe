...
<body style="width:97%;height:93%;" class="body-font">
		...
		<div class="nui-panel" id="panel" title="<%=I18nUtil.getMessage(request, "bps.wfclient.agent.Aproactivities") %>" style="margin-top:10px;width:100%;" > 
			<div id="dataGrid" url="org.gocom.bps.wfclient.agent.business.queryAgent.queryProActInfo.biz.ext"
	...
	<script type="text/javascript">
    	...
    	var name="<%=muo.getUserObject().getUserName()%>";
    	var userID="<%=muo.getUserObject().getUserId()%>";
        ...
        function onValueChanged(){
        	...
        	if(data.operation=="modify") {
    			grid.setUrl("org.gocom.bps.wfclient.agent.business.queryAgent.queryProActInfo.biz.ext");
    			grid.load({process:data.process,agentID:data.agentID});
    		} else {
    			grid.setUrl("org.gocom.bps.wfclient.agent.business.queryAgent.getProActInfo.biz.ext");
    	...
    	function doSave(e){
    		...
    		if(operation=="modify") {
    			...
    			target="org.gocom.bps.wfclient.agent.business.maintainAgent.modifyAgent.biz.ext";
    		}
    		else {
    			target="org.gocom.bps.wfclient.agent.business.addAgent.addAgent.biz.ext";  
    		}
       		nui.ajax({
         		 url:target,
          		 data:data,
          		 ...
          		 success:function(result) {
          		 	if(result.exception) {
          		 		var exception=result.exception.message;
          		 		var reason;          
    	...
    	function doCancel(e){
         ...
         function setData(data){
        	...
        	info.agentFromName="<%=muo.getUserObject().getUserName()%>";
        	info.operation="modify";
        	form.setData(info);
        	var json ={agentID:agentID,agentType:data.agentType,isModify:"Y"};
        	nui.ajax({
         		 url:"org.gocom.bps.wfclient.agent.business.queryAgent.queryAgentInfo.biz.ext",
          		 data:json,
          		 cache:false,
          		 success: function(result){
          		 	if(result.exception) { 
