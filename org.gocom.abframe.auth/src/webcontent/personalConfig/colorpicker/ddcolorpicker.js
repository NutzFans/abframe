...
definepicker:function(config){
	var colorpicker = new YAHOO.widget.ColorPicker(config.colorcontainer[1], {
		showcontrols: true, //set to true or false to toggle various controls
		...
		showhexcontrols: true,
		images: {
		PICKER_THUMB: contextPath + "/auth/personalConfig/colorpicker/picker_thumb.png", //path to picker thumb png
		HUE_THUMB: contextPath + "/auth/personalConfig/colorpicker/hue_thumb.png" //path to hue thumb png
		}
	})
...
init:function(config){
	...
	for (var i=0; i<config.fields.length; i++){
		...
		//config.colorfields[i].input.onfocus=function(){ //Behavior when focus is set on color input field
			window[config.colorcontainer[0]+'-active']=config.colorfields[i].input //make this field the currently active one (color picker writes to this field)
