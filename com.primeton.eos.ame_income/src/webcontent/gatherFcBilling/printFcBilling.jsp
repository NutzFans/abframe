...
<div class="nui-fit">
	<form id="form1" method="post" >
        ...
        <fieldset style="border:solid 1px #aaa;padding:0px;width: 1195px;">
        ...
        <fieldset style="border:solid 1px #aaa;padding:0px;width: 1135px;">
        ...
        <fieldset style="border:solid 1px #aaa;padding:0px;display: none;width: 1335px;" id="kpcpxxxx">
            ...
            <div style="width:100%;">
		        <div class="nui-toolbar" style="border-bottom:0;padding:0px;">
		            <table style="width:100%;">
		                <tr>
		                    <td style="width:100%;">
		                    	<a class="nui-button" onclick="invalid_prod()" iconCls="icon-remove" style="width:95px;margin-right:20px;display:none;" id="invalid_prod">产品作废</a>	
	                    	</td>
		                </tr>
		            </table>           
		        </div>
            ...
            <div style="width: 1330px;height: 25px;float:left;">
        ...
        <fieldset style="border:solid 1px #aaa;padding:0px;display: none;width: 1285px;" id="kpfwxxxx">
            ...
            <div style="width:100%;">
		        <div class="nui-toolbar" style="border-bottom:0;padding:0px;">
		            <table style="width:100%;">
		                <tr>
		                    <td style="width:100%;">
		                    	<a class="nui-button" onclick="invalid_serv()" iconCls="icon-remove" style="width:95px;margin-right:20px;display:none;" id="invalid_serv">服务作废</a>	
	                    	</td>
		                </tr>
		            </table>           
		        </div>
            ...
            <div style="width: 500px;height: 25px;margin-left: 65px;float:left;">
				<table style="table-layout:fixed;font-weight: bold;" border="0" cellspacing="0" cellpadding="0">
					<tr>
						...
						<!-- 产品开票金额 -->
						<td id="servMoneyTotal" style="width: 155px;" align="right" ></td>
					</tr>
				</table>
			</div>
        </fieldset>
        ...
        <fieldset style="border:solid 1px #aaa;padding:0px;width: 1050px;">
        ...
        <fieldset style="border:solid 1px #aaa;padding:0px;width: 1050px;">
        	...
        	<div style="width:100%;">
		        <div class="nui-toolbar" style="border-bottom:0;padding:0px;">
		            <table style="width:100%;">
		                <tr>
		                    <td style="width:100%;">
		                    	<a class="nui-button" iconCls="icon-add" onclick="addRow('ticket')" style="display:none;">增加</a>
		                    	<a class="nui-button" iconCls="icon-remove" onclick="removeRow('ticket')" style="display:none;">删除</a>
		                    	<a class="nui-button" onclick="saveTicks()" iconCls="icon-save" style="width:95px;margin-right:20px;display:none;" id="saveButton">保存发票</a>	
	                    	</td>
		                </tr>
		            </table>           
		        </div>
            ...
            <div style="height: 25px;float:left;">
				<table style="table-layout:fixed;font-weight: bold;" border="0" cellspacing="0" cellpadding="0">
					<tr>
						<td align="right" style="width: 305px;"></td>
						<!-- 发票金额 -->
...
<div style="text-align:center;padding:0px;border-width:1px 0px 0px 0px;" class="nui-toolbar" id="temp11" style="display:">
	<a class="nui-button" iconCls="icon-print" onclick="javascript:doPrint();" style="width:100px;margin-right:20px;display:none;">打印页面</a>
	<a class="nui-button" onclick="doViewSettle" id="viewSettle" style="width:100px;display:none;">查看流程图</a>
    <a class="nui-button" onclick="onCancel" style="width:60px;;margin-right:20px;display:none;">关闭</a>
    <a class="nui-button"  id="delete" onclick="deletes()" style="width:80px;display:none;">开票作废</a>
</div>
...
<script type="text/javascript">
    ...
    //页面进入时调用的方法--初始化
    function SetData() {
		var json = nui.encode({csNewbill:{billid: billid}});
        nui.ajax({
            ...
            success: function (data) {
            	...
            	document.getElementById("contsum").innerHTML = data.csNewbill.csContract.contsum == null ? "0.00" : toThousands(data.csNewbill.csContract.contsum,'#,##.##');
            	document.getElementById("salename").innerHTML = data.csNewbill.csContract.salename == null ? "" : data.csNewbill.csContract.salename;
            	document.getElementById("income").innerHTML = data.csNewbill.csContract.income == null ? "0.00" : toThousands(data.csNewbill.csContract.income,'#,##.##');
				...
				document.getElementById("csContract.servmon").innerHTML = data.csNewbill.csContract.servmon== null ? "0.00" : toThousands(data.csNewbill.csContract.servmon,'#,##.##');
                document.getElementById("csContract.prodmon").innerHTML = data.csNewbill.csContract.prodmon== null ? "0.00" : toThousands(data.csNewbill.csContract.prodmon,'#,##.##');
                document.getElementById("csContract.masum").innerHTML = data.csNewbill.csContract.masum== null ? "0.00" : toThousands(data.csNewbill.csContract.masum,'#,##.##');
				//合同收款记录
				...
				csGatherFcGrid.load({"csGatherFc": 
					...
					//开票产品金额
    				document.getElementById("billprodmon").innerHTML = data.csNewbill.billprodmon == null ? "0.00" : toThousands(data.csNewbill.billprodmon,'#,##.##');
    				//开票服务金额
    				document.getElementById("billservmon").innerHTML = data.csNewbill.billservmon == null ? "0.00" : toThousands(data.csNewbill.billservmon,'#,##.##');
    				//开票金额
    				document.getElementById("billsummon").innerHTML = data.csNewbill.billsummon == null ? "0.00" : toThousands(data.csNewbill.billsummon,'#,##.##');
    				//开票类型
    				...
    				billstatus = data.csNewbill.status;
					...
					//是否允许作废
					if(data.csNewbill.status != '1'){
						nui.get("delete").disable();
						nui.get("saveButton").disable();
						nui.get("invalid_prod").disable();
						nui.get("invalid_serv").disable();
					}
    				ticketSumRows = ticket_grid.getData().length;
    				
    				//查询审核意见
    				if(data.csNewbill.processinstid==null || data.csNewbill.processinstid == ''){
    					document.getElementById("datagrid1").style.display="none";
    					document.getElementById("opinionHide").style.display="none";
    				}else{
						var grid1 = nui.get("datagrid1");
						grid1.load({processInstID: data.csNewbill.processinstid});
    ...
    //打印页面
    function doPrint(){
    	document.getElementById("temp11").style.display = "none";
    	window.print();
    }
	...
	function deletes(){
		...
		for (var i = 0;i < csTickets.length;i++){
			...
			if(ticketid == null || ticketid == ""){
				nui.alert("作废后不能新增或冲正发票！");
				return;
			}
		}
		var rows = csGatherFcGrid.getSelecteds();
		var json = nui.encode({csNewbill:{billid: billid},csTickets:csTickets,csGatherFcs:rows,processinstid:processinstid});
		nui.confirm("作废后如需发票需再次发起开票流程！确定作废本开票记录么？","操作提示",function (action){
			if(action == "ok"){
				nui.ajax({
					url:"com.primeton.eos.ame_income.gatherFcBilling.deleteBillNew.biz.ext",
					data: json,
					type:"post",
					contentType:"text/json",
					success:function(o){
						o = nui.clone(o);
						if(o.exception == null){
							nui.alert("作废成功！","操作提示",function(){
								CloseWindow("ok");
							});
						}else{
							nui.alert("作废失败！");
							nui.get("delete").setEnabled(true);
						}
					},
	...
	function saveTicks(){
		...
		var json = nui.encode({csNewbill:{billid: billid,status:billstatus},csTickets:csTickets});
		nui.ajax({
			url:"com.primeton.eos.ame_income.gatherFcBilling.saveBillTickets.biz.ext",
			data: json,
			type:"post",
			contentType:"text/json",
			success:function(o){
				o = nui.clone(o);
				if(o.exception == null){
					nui.alert("发票保存成功！","操作提示",function(){
						var json = nui.encode({csNewbill:{billid: billid}});
				        nui.ajax({
				            url: "com.primeton.eos.ame_income.gatherFcBilling.getBillApplyNew.biz.ext",
							type: 'POST',
    ...
    function onActionRenderer(e) {
	    ...
	    var s = null;
	    if((record.status == "1" || record.status == "2") && record.ticketid != null){
	    ...
	    }else if(record.status == "3" && record.ticketid != null){
	    	s = '<a class="Edit_Button" style="padding-left: 20px;" href="javascript: editRow_zf(\'' + uid + '\')" >作废</a>'  ;
    ...
    //产品作废
    function invalid_prod(){
    	var rows = prod_grid.getSelecteds();
    	var json = nui.encode({csNewbill:{billid: billid},csBillProds:rows});
		if(rows.length > 0){
    		nui.get("invalid_prod").setEnabled(false);
			nui.confirm("确定作废选中的开票产品信息？", "操作提示",
	            function (action) {
	            	if (action == "ok") {
	            		nui.ajax({
							url:"com.primeton.eos.ame_income.gatherFcBilling.invalidProdServ.biz.ext",
							data: json,
							type:"post",
							contentType:"text/json",
							success:function(o){
								o = nui.clone(o);
								if(o.exception == null){
									nui.alert("开票产品信息作废成功！","操作提示",function(){
										var json = nui.encode({csNewbill:{billid: billid}});
								        nui.ajax({
								            url: "com.primeton.eos.ame_income.gatherFcBilling.getBillApplyNew.biz.ext",
											type: 'POST',
									...
									nui.alert("开票产品信息作废失败！");
								}
								nui.get("invalid_prod").setEnabled(true);
							},
							error:function(jqXHR, textStatus, errorThrown){
								nui.get("invalid_prod").setEnabled(true);
								nui.alert("error:" + jqXHR.responseText);
			...
			nui.alert("请至少选择一行开票产品信息！");
		}
    }
    
    //服务作废
    function invalid_serv(){
    	var rows = serv_grid.getSelecteds();
    	var json = nui.encode({csNewbill:{billid: billid},csBillSales:rows});
		if(rows.length > 0){
			nui.get("invalid_serv").setEnabled(false);
			nui.confirm("确定作废选中的开票服务信息？", "操作提示",
	            function (action) {
	            	if (action == "ok") {
	                	nui.ajax({
							url:"com.primeton.eos.ame_income.gatherFcBilling.invalidProdServ.biz.ext",
							...
							success:function(o){
								o = nui.clone(o);
								if(o.exception == null){
									nui.alert("开票服务信息作废成功！","操作提示",function(){
										var json = nui.encode({csNewbill:{billid: billid}});
								        nui.ajax({
								            url: "com.primeton.eos.ame_income.gatherFcBilling.getBillApplyNew.biz.ext",
											type: 'POST',
									...
									nui.alert("开票服务信息作废失败！");
								}
								nui.get("invalid_serv").setEnabled(true);
							},
							error:function(jqXHR, textStatus, errorThrown){
								nui.get("invalid_serv").setEnabled(true);
								nui.alert("error:" + jqXHR.responseText);
