								...
								<legend>合同收入金额信息</legend>
									...
									<table style="table-layout:fixed;">
										<tr>
											...
											<td style="width:100px" align="left">预计确认金额</td>
											<td style="width:100px" align="center">实际确认金额</td>
											<td style="width:80px" align="center">税率</td>
											<td style="width:100px" align="center">不含税金额</td>
										</tr>
										<tr> 
						...
						<div style="padding:5px;width: 515px" id="finreve" >
							<table style="table-layout:fixed;">
								...
								<tr id="setyearmonthFin1" style="display: none;">
									...
									<td align="right">本次实际工作量：</td>
									<td><input name="csReveForecastFin.labormonthact" id="laborcountinto2" required="true" class="nui-spinner" changeOnMousewheel="false" style="width:110px" format="n2" showbutton="false" minValue="-1000" maxValue="1000"/></td>
								</tr>
							</table>
							<fieldset id="field3" style="border:solid 1px #aaa;padding:3px;">
								<legend>合同收入金额信息</legend>
    ...
    <fieldset id="field4" style="border:solid 1px #aaa;padding:3px;">
			...
			<div id="grid_purCont" class="nui-datagrid" style="width:100%;height:auto;" allowResize="false" dataField="purContracts" ondrawsummarycell="doCountSum"
			 showPager="false" showSummaryRow="true">
				<div property="columns">
					<div type="indexcolumn" width="25"></div>	
					<div field="suppliersname" width="70" align="center" headerAlign="center">供应商简称</div>							
	        ...
	        <div id="grid_purSettle" class="nui-datagrid" dataField="purSettles" style="width:100%;height:auto" showSummaryRow="true"ondrawsummarycell="doCountSum"
	            ...
	            <div property="columns">
	                ...
	                <div field="purcontnum" width="50" align="center" headerAlign="center" renderer="getDetail">采购合同编号</div>
	            </div>
	        </div>
	        <br/>
	        <div class="nui-toolbar" style="border-bottom:0;padding:0px;">
			...
			<div id="grid_presettle" class="nui-datagrid" dataField="purpresettles" style="width:100%;height:auto" ondrawsummarycell="doCountSum"
	            ...
	            <div property="columns">
	                ...
	                <div field="purcontnum" width="60" headerAlign="center" align="center" renderer="getDetail1">采购合同编号</div>
	            </div>
	        </div>
	        <br/>
...
<script type="text/javascript">
	...
	function loadData(){
		...
		nui.ajax({
			...
			success:function (o){
				...
				nui.get("status").setValue("3");//状态默认为已审核确认
				//nui.get("status").setValue(o.csReveForecast.status);
				statusfrom = o.csReveForecast.status;
				/* if(o.csReveForecast.status == "2"){
					nui.get("status").setData([{"dictID":"2","dictName":nui.getDictText('AME_REVESTATUS',2)},{"dictID":"7","dictName":nui.getDictText('AME_REVESTATUS',7)},{"dictID":"6","dictName":nui.getDictText('AME_REVESTATUS',6)},{"dictID":"3","dictName":nui.getDictText('AME_REVESTATUS',3)}]);
				}else if (o.csReveForecast.status == "8"){
		    		nui.get("maconfirmday").setReadOnly(true);
					nui.get("status").setData([{"dictID":"8","dictName":nui.getDictText('AME_REVESTATUS',8)},{"dictID":"3","dictName":nui.getDictText('AME_REVESTATUS',3)}]);
				} */
				//合同基本信息 
                ...
                //财报确认日期默认管报确认日期
                if(o.csReveForecastFin.memo==null||o.csReveForecastFin.memo==""){
			    	nui.get("memo").setValue(o.csReveForecast.memo);
                }
                //财报确认日期默认管报确认日期
    ...
    function onOk(e) {
    	...
    	if(auditstatus == "1"){
			...
			actsum=parseFloat(actsum).toFixed(2);
	  		...
	  		if(summoney!=actsum){
	  			//防止计算出总金额之后又修改总金额
	  			alert("实际确认的产品、服务、MA总和不等于实际确认金额！");
	  			return;
	  		}
	  		//验证不含税
    ...
    function SaveData() {
    	...
    	if(syncflag=="true"){
    		...
    		FinReve.csReveForecastFin.status = "3";//确认环节状态为财务已确认
    	...
    	reve.csReveForecast.confirmdayStart = reve.csReveForecast.confirmdayStart=="" || reve.csReveForecast.confirmdayStart==null?reve.csReveForecast.confirmday:reve.csReveForecast.confirmdayStart; 
        ...
        var setend = document.getElementById("setend").innerHTML;
       	
        if((setend == null || setend == '') && reve.csReveForecast.maconfirmdayStart !='' && reve.csReveForecast.maconfirmdayStart !=null){
        	var maconfirmdayStart = new Date(reve.csReveForecast.maconfirmdayStart);
        	var curDateTime = maconfirmdayStart.getFullYear();
        	var month = maconfirmdayStart.getMonth()+1;
        	if(month>9){
				curDateTime = curDateTime + "" + month;
			}else{
				curDateTime = curDateTime + "0" + month;
			}
        	reve.csReveForecast.setend = curDateTime;
        }
    	var data_otherPurchases = grid_otherPurchase.getData();
        var json = nui.encode({csReveForecastFin:finreves,csReveForecast:reve.csReveForecast,misOpinion:opin.misOpinion,otherPurchases:data_otherPurchases});
        nui.ajax({
	...
	//选择供应商弹窗
		...
		function countprofit(){
	    	...
	    	document.getElementById("laborcountinto1").innerHTML = laborcountinto == null || laborcountinto == 0?"":formatNumber(laborcountinto,'#,##.##')+"人月";
	    	//nui.get("laborcountinto2").setValue(laborcountinto);
	    ...
	    function changestatus(){
	    	...
	    	if(status == "6"){//仅财报确认
	    		nui.get("maconfirmday").setReadOnly(true);
	    		if(nui.get("maconfirmday").getValue() != null && nui.get("maconfirmday").getValue() != ""){
	    			maconfirmday = nui.get("maconfirmday").getValue();
	    		}
	    		nui.get("maconfirmday").setValue(null);
	    		nui.get("maconfirmday").setRequired(false);
	    		
	    		nui.get("confirmday").setReadOnly(false);
	    		nui.get("confirmday").setRequired(true);
	    		if(nui.get("confirmday").getValue() == null || nui.get("confirmday").getValue() == ""){
	    			nui.get("confirmday").setValue(confirmday);
	    		}
	    	}else if (status == "7"){//仅管报确认
	    		nui.get("confirmday").setReadOnly(true);
	    		if(nui.get("confirmday").getValue() != null && nui.get("confirmday").getValue() != ""){
	    			confirmday = nui.get("confirmday").getValue();
	    		}
	    		nui.get("confirmday").setValue(null);
	    		nui.get("confirmday").setRequired(false);
	    		
	    		nui.get("maconfirmday").setReadOnly(false);
	    		nui.get("maconfirmday").setRequired(true);
	    		if(nui.get("maconfirmday").getValue() == null || nui.get("maconfirmday").getValue() == ""){
	    			nui.get("maconfirmday").setValue(maconfirmday);
	    		}
	    	}else if(status == "3"){//财报管报已确认
	    		if(statusfrom == "2"){//收入确认流程中
		    		nui.get("maconfirmday").setReadOnly(false);
		    		nui.get("maconfirmday").setRequired(true);
	    			if(nui.get("maconfirmday").getValue() == null || nui.get("maconfirmday").getValue() == ""){
		    			nui.get("maconfirmday").setValue(maconfirmday);
	    		...
	    		}else if (statusfrom == "8"){//管报已确认的财报确认流程中
		    		nui.get("maconfirmday").setReadOnly(true);
		    		nui.get("maconfirmday").setRequired(true);
		...
		function selectFinreve(){
			...
			var malabormonth = document.getElementById("labormonth").innerHTML;//本次结算工作量;
			...
			nui.get("laborcountinto2").setValue(malabormonthact-labormonthact);
		...
		function syncjysr(e){
			nui.confirm("是否确认将经营收入覆盖财务收入？","操作提示",function(o){
				if("ok"==o){
					...
					nui.get("laborcountinto2").setValue(malabormonthact);
					calThreeMoneyFin();//计算不含税
				}
			})
		}
