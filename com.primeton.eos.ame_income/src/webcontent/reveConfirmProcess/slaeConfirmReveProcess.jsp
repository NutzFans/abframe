			...
			<div style="padding:5px;">
				...
				<table style="table-layout:fixed;">
					...
					<tr id="setyearmonth" style="display: none;">
						...
						<td style="width:160px"><input name="csReveForecast.setstart" id="setstart" required="true" class="nui-monthpicker" format="yyyyMM" style="width:150px" allowInput="false" onvaluechanged="selectYM"/></td>
						<td align="right" style="width:100px">本次结算结束年月：</td>
						<td style="width:160px"><input name="csReveForecast.setend" id="setend" required="true" class="nui-monthpicker" format="yyyyMM" style="width:150px" allowInput="false" onvaluechanged="selectYM"/></td>
...
<script type="text/javascript">
	...
	function loadData(){
		...
		nui.ajax({
			...
			success:function (o){
	            ...
	            changepurChase();
	            //alert(o.csReveForecast.project);
   ...
   function  calThreeMoney(){
    ...
    function onOk(e) {
    	...
    	if(nui.get("auditstatus").getValue() == 1 && a.length < 1){
    ...
    function SaveData() {
        ...
        var json = nui.encode({csReveForecast:reve.csReveForecast,misOpinion:opin.misOpinion,otherPurchases:data_otherPurchases,purChaseYesOrNo:purChaseYesOrNo});
        nui.ajax({
	...
	//选择供应商弹窗
		...
		function doViewaccruedmoney(contnum,rlny){
			var startdate = rlny.substring(0,4)+'-'+rlny.substring(4,6)+'-01';
			var enddate = rlny.substring(0,4)+'-'+rlny.substring(4,6)+'-'+getLastDayOfMonth(rlny.substring(0,4),rlny.substring(4,6));
			var executeUrl = "/default/ame_income/reveConfirmProcess/purCostdetail.jsp?contnum=" + contnum+"&startdate="+startdate+"&enddate=";
