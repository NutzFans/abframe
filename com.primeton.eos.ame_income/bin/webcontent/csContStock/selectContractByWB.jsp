		        ...
		        <table style="width:100%;">
		              <tr>
		                  <td style="width:100%;">
		                  	 ...
		                  	 <a class="nui-button" iconCls="icon-download" onclick="exportWB()">导出</a>
		                  </td>
		              </tr>
		         </table>
		</div>
		<div id="datagrid1" class="nui-datagrid" style="width:auto;height:auto;" sizeList="[50,100,200]" pageSize="200"
	            url="com.primeton.eos.ame_income.csContStock.selectContractByWB.biz.ext" dataField="cs" onSelect="changeC"
	            multiSelect="true" showSummaryRow="true" ondrawsummarycell="onDrawSummaryCell" ondrawcell="ondrawcell">
	        <div property="columns">
	        	<div type="checkcolumn" width="20"></div>
	        	<div field="contnum" width="50" headerAlign="center" align="center">合同编号</div>
	        	...
	        	<div field="notaxmon" width="50" headerAlign="center" align="center" dataType="currency" summaryType="sum">采购成本</div>
	        </div>
	    </div>
	</div>
...
<script>
	...
	function changeC(){
		...
		if(row.costmemo!='1' && row.costmemo!='0'){
			grid.deselect(row);
		}
	}
	...
	function onDrawSummaryCell(e) {
	    ...
	    if (e.field == "notaxmon") {
		    ...
		    if(e.cellHtml!=null&&e.cellHtml!=""){
		    	notaxmon = e.cellHtml;
		    	notaxmon = notaxmon.replace(/,/g,"");
			    for(var i=0;i<result.cs.length;i++){
			    	if(result.cs[i].contnum == "售前"){
			    		sq = result.cs[i].notaxmon
			    	}else if(result.cs[i].contnum == "组织级"){
			    		zzj = result.cs[i].notaxmon
			    	}else if(result.cs[i].contnum == "研发"){
			    		yf = result.cs[i].notaxmon
			    	}else if(result.cs[i].contnum == "资产"){
			    		zc = result.cs[i].notaxmon
			    	}else if(result.cs[i].contnum == "非项目成本"){
			    		fxm = result.cs[i].notaxmon
			    	}
			    }
			    if(sq!=null&&sq!=""){
			    	rg = (parseFloat(notaxmon) - parseFloat(sq)).toFixed(2);
			    }
			    if(zzj!=null&&zzj!=""){
			    	rg = parseFloat(rg) - parseFloat(zzj);
			    }
			    if(yf!=null&&yf!=""){
			    	rg = parseFloat(rg) - parseFloat(yf);
			    }
			    if(zc!=null&&zc!=""){
			    	rg = parseFloat(rg) - parseFloat(zc);
			    }
			    if(fxm!=null&&fxm!=""){
			    	rg = parseFloat(rg) - parseFloat(fxm);
			    }
		        e.cellHtml = "<span style='display: block;font-weight: bold;font-size: 12px;text-align: right;'>" + rg + "</br>" + notaxmon + " </span>";
		    }else{
    ...
    //导出
    function exportWB(){
		if(!confirm("导出采购成本归集记录？")){
			return;
		}
		var json =  nui.encode({"year":year,"month":month});
		nui.ajax({
			url: "com.primeton.eos.ame_income.csContStock.exportWB.biz.ext",
	        type: "post",
	        data: json,
	        cache: false,
