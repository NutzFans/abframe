...
<!-- 
  - Author(s): 朱勇伦
  - Date: 2016-12-20 15:04:13
  - Description:
-->
<head>
	<title>合同收款开票流程-销售确认发票送达客户</title>
	            ...
	            <div style="padding:5px;">
		        	<div id="prod_grid" class="nui-datagrid" style="width: 100%;height:auto;" allowResize="true" 
				        ...
				        <div property="columns">
				            ...
				            <div field="num" width="60" headerAlign="center" align="right" dataType="currency">开票数量</div>
...
<script type="text/javascript">
	...
	function init(){
		...
		nui.ajax({
			...
			success:function(data){
				...
				//流程回退路径赋值 
				var backList = data.csNewbill.backList;
				for(var i = 0;i < backList.length;i ++){
    				if(backList[i].id == "autoActivity1"){
    					delete backList[i];
    				}
				}
	...
	function SaveData(){
		...
		processSaleSureDelivered();
	}
	
	function processSaleSureDelivered(){
    	var json = form_cont.getData();
    	//开票产品信息
		json.csBillProds = prod_grid.getData();
		//开票服务信息
		json.csBillSales = serv_grid.getData();
		//工作项ID
    	...
    	nui.ajax({
			url:"com.primeton.eos.ame_income.gatherFcBillingProcess.gatherFcBillingProcessSaleSureDelivered.biz.ext",
			data: json,
			type:"post",
			contentType:"text/json",
			success:function(o){
