...
function menuListRefresh(node){
 ...
 function menuClick(node){
  ...
  var openmode = node.getProperty("openmode");
  if(!node.hasChildNode()){
     if(url != null && openmode != "new" && openmode != "pop"){       
        parent.frames[target].location.href = generateUrl(url,appid,'<%=request.getContextPath() %>');
     }else if(url != null && openmode == "new"){
     	window.open(generateUrl(url,appid,'<%=request.getContextPath() %>'));
     }else if(url != null && openmode == "pop"){
    	window.open(generateUrl(url,appid,'<%=request.getContextPath() %>'),'newWin','width=750,height=220,top=180,left=200,toolbar=no,location=no,directories=no,status=no,menubar=no');
     }
  }else{
  	node.expandNode();
  }
