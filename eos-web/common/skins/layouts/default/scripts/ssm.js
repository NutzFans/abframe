...
if (NS6) {ssm.top = (parseInt(ssm.left)-num)+"px";}
if (NS) {ssm.top = (parseInt(ssm.left)-num); bssm.clip.right+=num;bssm2.clip.right+=num;}
}
}
...
for(i=0;i<ssmItems.length;i++) {
	if(ssmItems[i][1] != "undefined" && ssmItems[i][1] != null){		
		if(! protoexpr.test(ssmItems[i][1])){ //如果是http或者https开头则直接返回
			if(ssmItems[i][1].substring(0,1) == "/"){
				ssmItems[i][1] = "/default"+ssmItems[i][1];
			}else{
				ssmItems[i][1] = "/default/"+ssmItems[i][1];
			}
	    }
...
if(!ssmItems[i][1]){
document.write('<td bgcolor="'+hdrBGColor+'" HEIGHT="'+hdrHeight+'px" ALIGN="'+hdrAlign+'" VALIGN="'+hdrVAlign+'" WIDTH="'+ssmItems[i][5]+'" COLSPAN="'+ssmItems[i][3]+'">&nbsp;<font face="'+hdrFontFamily+'" Size="'+hdrFontSize+'" COLOR="black"><b>'+ssmItems[i][0]+'</b></font></td>')}
