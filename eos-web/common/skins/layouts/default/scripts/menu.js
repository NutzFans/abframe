...
function generateUrl(url,appid,context){
    ...
    if(protoexpr.test(url)){ //如果是http或者https开头则直接返回
	    ...
	    if(app!=null&&app.apptype!='0'){
	      ...
	      //alert("remote:  http://"+app.ipaddr+generatePort(app.ipport)+"/"+generateContext(app.url,context)+"/"+geturl(url));
	      return "http://"+app.ipaddr+generatePort(app.ipport)+"/"+generateContext(app.url,context)+"/"+geturl(url);
	    }else{
	      //alert("local: /"+generateContext(null,context)+"-/-"+geturl(url));
	      return "/"+generateContext(null,context)+"/"+geturl(url);
	    }
    }
}
 ...
 function generateContext(newcontext,currentContext){
