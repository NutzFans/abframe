...
<head>
	<title>
	<% if(request.getParameter("raq").equals("/csContractRecognisedIncome.raq")){ %>部门合同收入统计报表
	<%}else{ %>报表显示
	<%} %></title>
...
//	alert("report1:"+jQuery('#report1_scrollArea').offset().top);
	//取操作日志表中名称为“生成外包费用统计”的最近处理时间
	var ajax = new Ajax("com.primeton.eos.ame_pur.settle.newDoDate.biz");
	ajax.submit();
	//将返回值字符串化
	var newDoDate = ajax.getValues("root/data/newDoDate") + "";
	//将字符串化的最近处理时间塞入标签
	document.getElementById("newDoDate").innerHTML = newDoDate.substring(0,19);
...
<%
	...
	if(paramNames!=null){
		...
		if (paramFile.equals("/sales_performance_arg.raq")||paramFile.equals("/sales_performance_new_arg.raq")
		 || paramFile.equals("/ame_projectcostsum_arg.raq")
		|| paramFile.equals("/csContractRecognisedIncome_arg.raq") || paramFile.equals("/csContractRecognisedIncomeByYear_arg.raq")
		|| paramFile.equals("/csContractDepartmentRecord_arg.raq") 
		//外包费用统计加权限
		|| paramFile.equals("/outfeestat2_arg.raq") || paramFile.equals("/outfeestatByConfirmdate_arg.raq") 
		|| paramFile.equals("/csGatherFc_arg.raq") || paramFile.equals("csContractReveCostAccounting_arg.raq")
		|| paramFile.equals("/sales_keyPerformance_arg.raq")||paramFile.equals("/sales_keyPerformance_new_arg.raq")
		|| paramFile.equals("/ame_AccountsReceivable_arg.raq") || paramFile.equals("/ame_ReceivableUnearned_arg.raq")
		|| paramFile.equals("/ame_cashflow_arg.raq")|| paramFile.equals("/emp_detail_arg.raq")
		|| paramFile.equals("/emp_month_arg.raq") || paramFile.equals("/ame_labordateconut_arg.raq") || paramFile.equals("/ame_laborrate_arg.raq")){
	...
	%>
		<table id="param_tbl"><tr><td>
			<tr>
					...
					<td>
						<!-- 外包费用统计报表新 -->
						<%-- <% if(report.equals("/outfeestat2.raq")){ 
						...
						<%}%> --%>
						<!-- 外包费用统计报表 -->
						<% if(report.equals("/outfeestat0803.raq") || report.equals("/outfeestat2.raq") || report.equals("/outfeestatByConfirmdate.raq")){ 
							StringBuffer param1=new StringBuffer();
							String reportParamsId=request.getParameter("reportParamsId");
							Hashtable paramtest = null;
							Date date = new Date();
						...
						%>
							...
							（最近数据预处理时间：<span id="newDoDate"></span>）
							<br/>
						<%}%>
						<a href="javascript:_submit( form1 )" ><img src="../images/search.gif" border=no style="vertical-align:middle"></a>
										...
										<%
                            if(report.equals("/ps_overall1.raq")){ %>
                            <td><input type="button" onclick="doUpdateID()" value="生成对应ID"></td>
                            <% }%>
							
							<%
                            if(report.equals("/deptBudget.raq")){ 
                             String reportParamsId=request.getParameter("reportParamsId");
                               Hashtable paramtest = null;
                               String year = "";
                              if(!"".equals(reportParamsId) && reportParamsId != null){
                                ...
                                year = (String)paramtest.get("year"); 
                            }
                               %>
                            <td><input type="button" onclick="impDeptBudgetData(<%=year %>)" value="导入预算数据"></td>
                            <% 
                            }%>
                            ...
                            <%
                            if(report.equals("/sales_performance.raq")||report.equals("/sales_performance_new.raq")){ 
                               StringBuffer param1=new StringBuffer();
                               String reportParamsId=request.getParameter("reportParamsId");
                               Hashtable paramtest = null;
                              if(!"".equals(reportParamsId) && reportParamsId != null){
                               ...
                               String year = (String)paramtest.get("year"); 
                               String startmonth = (String)paramtest.get("startmonth"); 
                               String endmonth = (String)paramtest.get("endmonth"); 
                               String orgseq = (String)paramtest.get("orgseq"); 
                               UserObject user = (UserObject) request.getSession().getAttribute("userObject");
							   String userid = user.getUserId();
                               int action = Integer.parseInt((String)paramtest.get("action"));
                               System.out.print("xuqingqian"+year+","+startmonth+","+endmonth+","+orgseq+","+userid);
   
                               %>
                            <td><input type="button" onclick="sales_performence_export('<%=year %>','<%=startmonth %>','<%=endmonth %>','<%=orgseq %>','<%=userid %>','<%=action %>')" value="导出本部门及下级部门"></td>
                            <% }}%>
                            
                            <%
                            if(report.equals("/deptBudget.raq")||report.equals("/deptBudgerNew.raq")){ 
                               StringBuffer param1=new StringBuffer();
                               String reportParamsId=request.getParameter("reportParamsId");
                               Hashtable paramtest = null;
                              if(!"".equals(reportParamsId) && reportParamsId != null){
                               ...
                               String orgid = (String)paramtest.get("orgid");  
                               String year = (String)paramtest.get("year"); 
                               UserObject user = (UserObject) request.getSession().getAttribute("userObject");
							   String userid = user.getUserId();
                               int action = Integer.parseInt((String)paramtest.get("action"));
                               System.out.print("xuqingqian"+year+","+orgid);
   
                               %>
                            <td><input type="button" onclick="deptBudget_export('<%=orgid %>','<%=year %>','<%=action %>')" value="导出本部门及下级部门"></td>
                            <% }}%>
				</tr>
</table>
	</td></tr>
