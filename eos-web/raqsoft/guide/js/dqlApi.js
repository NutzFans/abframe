	...
	obj._refreshLmd = function() {
		...
		if (dct != null) {
			for (var i=0; i<dct.tableItemList.length; i++) {
				...
				if (t == null) continue;
				t.dispName = ti.name;
				for (var j=0; j<ti.dataItemList; j++) {
...
function Olap(){
			...
			if (conf.fields.length==0) {
				...
				for (var i=0; i<conf.tops.length; i++) {
					...
					if (sgts.tables.length>1) {
						...
						alert("只有来自不同表的维字段，不能拼出正确dql，请拖拽一些测度字段到数据区");
						return null;
					}
			...
			for (var i=0; i<conf.fields.length; i++)
			{
				...
				if (tj == null) {
						if (sgts.tables.length>0) {
							alert("多表查询时，需要拖拽一些共有维的字段到左/上表头，才能形成有意义的查询");
							return null;
						}
			...
			if (dataSet.type == 6){
				try {
					...
					if (dataSet.tableName.indexOf(iObj.firstTable) == -1){
						f._status = "数据集结构已变动1，["+f.name+"]字段失效";
					}
					...
					if (f._parentType != 'field') {
						...
						if (invalidTable != "") f._status = "维【"+f.dim+"】和表【"+invalidTable+"】无法关联上";
					}
					
					alert = alertBack;
				} catch (e) {
					console.warn(e);
					f._status = "数据集结构已变动2，["+f.name+"]字段失效";
				}
			} else {
				var src = obj.dataSetUtils.getDataSetField(dataSet,f.srcName);
				if (!src) f._status = "数据集结构已变动3，["+f.name+"]字段失效";
			}
			if (name == '')
			{
	...
	var func4 = function(conf) {
		...
		var zongji = {"name":"总计","backColor":"204,255,255","color":-16777216,"hAlign":-47};
					...
					if (fields[i]._fieldType == 'aggr' && fields[i].groups == null) {
						if (conf.isRowData) {
							//if (tops.length>0){
								...
								newf.showName = onlyZongji?"总计":(newf.name+"-总计");
