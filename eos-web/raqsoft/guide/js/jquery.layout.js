 ...
 */
	...
	,	defaults: { // default options for 'all panes' - will be overridden by 'per-pane settings'
		...
		,	draggableIframeFix:		true		// Pass-through option for ui.draggable - true = mask ALL iframes onDrag
		...
		*/
		/*	these options MUST be set 'by pane'
		,	paneSelector:			""			// MUST be pane-specific!
 ...
 */
	...
	var execUserCallback = function (pane, v_fn) {
		...
		try {
			...
			if (typeof fn=="function")
				// pass back data: pane-name, pane-element, pane-state, pane-options, and layout-name
				fn( pane, $Ps[pane], $.extend({},state[pane]), $.extend({},options[pane]), options.name );
		}
		catch (ex) {}
	};
	...
	var cssNum = function ($E, prop) {
		...
		,	visibility = ""
		;
		if (!/msie/.test(navigator.userAgent.toLowerCase())) { // IE CAN read dimensions of 'hidden' elements - FF CANNOT
 ...
 */
	...
	var create = function () {
		...
		$(window).resize(function () {
			...
			window[timerID] = null;
			if (true || /msie/.test(navigator.userAgent.toLowerCase())) // use a delay for IE because the resize event fires repeatly
		...
		try { // format html/body if this is a full page layout
			if ($Container[0].tagName == "BODY") {
				...
				if (!p || "fixed,absolute,relative".indexOf(p) < 0)
					CSS.position = "fixed"; // container MUST have a 'position'
				if (!h || h=="auto")
	...
	var initOptions = function () {
		...
		// remove any 'default options' that MUST be set 'per-pane'
		$.each("paneSelector,resizerCursor,customHotkey,onopen,onclose,onresize".split(","),
		...
		$.each(c.borderPanes.split(","), function(i,pane) {
			...
			var o = options[pane] = $.extend( true, {}, options.defaults, options[pane], opts.defaults, opts[pane] );
			...
			$.each("onopen,onclose,onresize".split(","), function (idx,key) { 
			...
			var fxName = o.fxName;
			if (fxName != "none") {
				if (!fxName || !$.effects || !$.effects[fxName] || (!fx[fxName] && !o.fxSettings))
					o.fxName = "none"; // effect not loaded, OR undefined FX AND fxSettings not passed
				else if (fx[fxName])
					// ADD 'missing keys' to s.fxSettings from config.fxSettings
					o.fxSettings = $.extend( {}, fx[fxName].all, fx[fxName][pane], o.fxSettings );
			}
		});
	...
	var initPanes = function () {
		...
		$.each(c.allPanes.split(","), function() {
			...
			,	sel		= o.paneSelector
			,	isIE6	= (/msie/.test(navigator.userAgent.toLowerCase()) && $.browser.version < 7)
			,	CSS		= {}
			,	$P, $C
			;
			$Cs[pane] = false; // init
	...
	var initHandles = function () {
		...
		$.each(c.borderPanes.split(","), function() {
			...
			;
			if ($R[0].offsetWidth > $R[0].offsetHeight) {
			} else {
			}
			 // ADD VISUAL STYLES
			...
			if (o.closable) {
				...
				setTimeout(function(){
					if (isOpen) {
						if ($T[0].offsetWidth > $T[0].offsetHeight) {
							if ("south" == pane) $T[0].style.backgroundImage = "url('" + consts.relaPath + "/img/toward_down.gif')";
							else $T[0].style.backgroundImage = "url('" + consts.relaPath + "/img/toward_up.gif')";
							$R[0].style.backgroundImage = "url('" + consts.relaPath + "" + consts.imgFolder + consts.img1 + "')";			
						} else {
							if ("east" == pane) $T[0].style.backgroundImage = "url('" + consts.relaPath + "/img/toward_right.gif')";
							else if ("west" == pane) $T[0].style.backgroundImage = "url('" + consts.relaPath + "/img/toward_left.gif')";
							$R[0].style.backgroundImage = "url('" + consts.relaPath + "" + consts.imgFolder + consts.img2 + "')";			
						}
						$T[0].style.backgroundRepeat = "no-repeat";
					} else {
						if ($T[0].offsetWidth > $T[0].offsetHeight) {
							if ("south" == pane) $T[0].style.backgroundImage = "url('" + consts.relaPath + "/img/toward_up.gif')";
							else $T[0].style.backgroundImage = "url('" + consts.relaPath + "/img/toward_down.gif')";
							$R[0].style.backgroundImage = "url('" + consts.relaPath + "" + consts.imgFolder + consts.img1 + "')";			
						} else {
							if ("east" == pane) $T[0].style.backgroundImage = "url('" + consts.relaPath + "/img/toward_left.gif')";
							else if ("west" == pane) $T[0].style.backgroundImage = "url('" + consts.relaPath + "/img/toward_right.gif')";
							$R[0].style.backgroundImage = "url('" + consts.relaPath + "" + consts.imgFolder + consts.img2 + "')";			
						}
						$T[0].style.backgroundRepeat = "no-repeat";
					}
			...
			//$R.css('background-image', "url('" + consts.relaPath + "/img/split_v.gif')");
		});
	...
	var initResizable = function () {
		...
		$.each(c.borderPanes.split(","), function() {
			...
			,	start: function (e, ui) {
					...
					//$.each(o.panesToMaskOnResize.split(","), maskPane);
				}
			...
			,	stop: function (e, ui) {
					...
					//$.each(o.panesToMaskOnResize.split(","), unmaskPane);
	 ...
	 * maskPane / unmaskPane
	 *
	 * NOT USED - see options.draggableIframeIFix - but left here for future reference
	 */
	function maskPane (i,pane) {
		var $P = $Ps[pane];
		if (!$P) return; // invalid pane-name or pane does not exist
		$("<div></div>")
			.attr("id","layout_"+ state.id +"_"+ pane +"_mask")
			.css( $P.offset() ) // set top & left
			.css({
				position:	"absolute"
			,	width:		$P.outerWidth()
			,	height:		$P.outerHeight()
			,	zIndex:		c.zIndex.mask
			})
			.appendTo("body")
		;
	}
	function unmaskPane (i,pane) {
		$("#layout_"+ state.id +"_"+ pane +"_mask").remove();
	}
 ...
 */
	...
	var close = function (pane, force, noAnimation) {
		...
		,	o		= options[pane]
		,	s		= state[pane]
		,	doFX	= !noAnimation && !s.isClosed && (o.fxName != "none")
		,	edge	= c[pane].edge
		,	rClass	= o.resizerClass
		,	tClass	= o.togglerClass
		,	_side	= "-"+ pane // used for classNames
		...
		if ($R) {
			...
			if ($T) {
				...
				;
				if ($T[0].offsetWidth > $T[0].offsetHeight) {
					if (pane == "south") $T[0].style.backgroundImage = "url('" + consts.relaPath + "/img/toward_up.gif')";
					else $T[0].style.backgroundImage = "url('" + consts.relaPath + "/img/toward_down.gif')";
				} else {
					if (pane == "west") $T[0].style.backgroundImage = "url('" + consts.relaPath + "/img/toward_right.gif')";
					else if (pane == "east") $T[0].style.backgroundImage = "url('" + consts.relaPath + "/img/toward_left.gif')";
				}
				$T[0].style.backgroundRepeat = "no-repeat";
			}
		...
		if (doFX) {
			...
			$P.hide( o.fxName, o.fxSettings, o.fxSpeed, function () {
		...
		function close_2 () {
			...
			execUserCallback(pane, o.onclose); // see if there is a callback for this pane

			// CHECK FOR internal flow-control callback
	...
	var open = function (pane, slide, noAnimation) {
		...
		if (doFX) { // ANIMATE
			lockPaneForFX(pane, true); // need to set left/top so animation will work
			$P.show( o.fxName, o.fxSettings, o.fxSpeed, function() {
				lockPaneForFX(pane, false); // undo
		...
		function open_2 () {
			...
			if ($R) {
				...
				if ($T) {
					...
					;
					if ($T[0].offsetWidth > $T[0].offsetHeight) {
						if (pane == 'south') $T[0].style.backgroundImage = "url('" + consts.relaPath + "/img/toward_down.gif')";
						else $T[0].style.backgroundImage = "url('" + consts.relaPath + "/img/toward_up.gif')";
					} else {
						if (pane == 'west') $T[0].style.backgroundImage = "url('" + consts.relaPath + "/img/toward_left.gif')";
						else if (pane == 'east') $T[0].style.backgroundImage = "url('" + consts.relaPath + "/img/toward_right.gif')";
					}
					$T[0].style.backgroundRepeat = "no-repeat";
				}
			...
			// see if there is a callback for this pane
			execUserCallback(pane, o.onopen);
	...
	var sizePane = function (pane, size) {
		...
		sizeHandles();
		sizeContent(pane);
		execUserCallback(pane, o.onresize);
	};
	...
	var sizeMidPanes = function (panes, overrideDims, onInit) {
		...
		$.each(panes.split(","), function() {
			...
			if (pane == "center") {
				 ...
				 */
				if (/msie/.test(navigator.userAgent.toLowerCase()) && (!$.boxModel || $.browser.version < 7)) {
