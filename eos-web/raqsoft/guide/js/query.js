			...
			callback: {
				...
				,onDrop: function(e, treeId, treeNodes, targetNode, moveType) {
					...
					if (treeNodes[0].level == 0 && !$.cookie('guideShowType') != '0') {
						...
						var iiObj = cus.getInfosObj(ss[0]);
						...
						//alert(aggr);
						var name = cus.generateNewFieldAlias(cus.getInfosObj(ss[0]));
						//if (aggr != '') {
...
$(document).ready(function(){
	...
	lmd = dqlQuery.lmd;
	if (dqlQuery.lmd != null)	lmds.push({dsName:guideConf.dataSource,lmd:dqlQuery});
...
//		});
		...
		if (olapStr != '') {
			...
			for (var ii = 0; ii<rqAnalyse.rpxs.length; ii++) {
				if (!rqAnalyse.rpxs[ii].reportId) rqAnalyse.rpxs[ii].reportId = "rid"+ new Date().getTime();
