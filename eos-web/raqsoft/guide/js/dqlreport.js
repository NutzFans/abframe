...
var aly = {
	...
	,queryDataSet : function(reQuery,dataSet,callback) {
		...
		olapObj.server({action:2,oper:'query',reQuery:reQuery,dataId:ds.dataId,type:ds.type,dataSource:ds.dataSource,ql:ds.ql,dfxFile:ds.dfxFile,dfxScript:ds.dfxScript,dfxParams:ds.dfxParams,inputFiles:ds.inputFiles,filter:'',currTable:ds.currTable,dataFileType:guideConf.dataFileType},function(data){
			if (data.indexOf('error:')==0) {
	...
	,showDataSetRpxs : function(dataSet,lastRpxFunc) {
		...
		if (guideConf.showHistoryRpx != 'yes'){
			...
			if (ds.type!=6 && ds.fields == null) {
				olapObj.server({action:2,oper:'getTableInfo',dataId:ds.dataId,dataFileType:guideConf.dataFileType},function(data){
						if (data.indexOf('error:')==0) {
							ds._status = data.substring(6);
							return;
						}
	...
	,refresh : function(noCalc, noRefreshDialog) {
		...
		if (conf.type == 1) { //自定义报表
			...
			//if (conf.lefts.length == 0) 
				div2.append("<div id='confHints' style='text-align: center; margin:3px;padding:3px;color:lightgray;'>左表头</div>");
	...
	,confField : {
		...
		,where : function(having){
			...
			//alert(dataScope+"222");
			var filter1 = '';
			if (dataSet.type == 6) filter1 = whereUtils.getExp(having==2?field.having.conf:field.where.conf, "T1.", 1);
			...
			var saveFunc = function () {
				...
				if (dataSet.type == 6) exp = whereUtils.getExp(having==2?field.having.conf:field.where.conf, "T1.", 1);
			...
			if (dataSet.type == 6){
				...
				if (having == 2) {
					...
					fields = [transWhereInfo(fd,null,true)];
				} else {
					if (field._fieldType == 'aggr'||field._fieldType == 'detail') {
						cus.getFieldInfos(iObj.firstTable, fields, 0, null, null,true);
						for (var i=0; i<fields.length; i++) fields[i] = transWhereInfo(fields[i],null,false);
						initField = transWhereInfo(field.src,null,false);
					} else fields = [transWhereInfo(field.src,field.name,false)];
			...
			whereUtils.openWhereDialog(saveFunc,clearFunc);
			whereUtils.refresh(fields, initField, JSON.parse(JSON.stringify(having==2?field.having.conf:field.where.conf)));
		...
		,rela : function() {
			...
			for (var i=0; i<usedTables.length; i++) {
				...
				} else $('#editRelaDiv').append(tObj.dispName+"&nbsp;:&nbsp;无法关联到维度["+dimObj.dispName+"]<br>");
			...
			function showAn() {
				...
				for (let i=0; i<currAnalyse.items.length; i++) {
					...
					let showCond = function () {
						...
						divii.html("");
						divii.append('<input type="text" id="analyseItemExp" placeholder="查找条件表达式" style="width:760px;height:26px;" value="'+itemi.exp+'">');
						let fs = [];
	...
	,generateReport : function(conf) {
		...
		var olapRpx = function() {
			...
			$.ajax({
				...
				async : true,
				url: contextPath + "/DLServletAjax?d="+new Date().getTime(),
				data: {action:2,oper:'genReport',reportId:conf.reportId,title:conf.name,rpt:JSON.stringify(rpt).replaceAll('"','<d_q>'),lefts:rpt.lefts,tops:rpt.tops,maxSize:guideConf.maxReportSize,resultRpxPrefixOnServer:guideConf.resultRpxPrefixOnServer},
				//confs:[{type:1自定义分析报表/2模板报表,name:'报表名称',reportId:'',template:'',show:0/1,isRowData:1,lefts:[{name:'',item:itemId,use:1,order:0无序/1升序/2降序}],tops:[],fields:[],wheres:[{item:itemId}]}...] 调序、排序、改名
				success: success
			});
		}
	...
	,queryData : function(conf) {
		...
		if (dataSet.type == 6){
			...
			$.ajax({
				...
				async : false,
				url: contextPath + "/DLServletAjax?d="+new Date().getTime(),
				data: {action:2,oper:'queryDqlData',reportId:conf.reportId,dqlSegments:dql,dataSource:dataSet.dataSource,maxDataSize:guideConf.maxDataSize},
				success: function(data){
					if (data.indexOf('error:')==0) {
						alert(data.substring(6));
						return;
	...
	,calcDfxData : function(dataSetField, conf, callback) {
		...
		function f() {
			$.ajax({
				...
				async : false,
				url: contextPath + "/DLServletAjax?d="+new Date().getTime(),
				data: {action:2,oper:'calc',reportId:reportId,dataId:dataId,cacheType:cacheType,calcs:calcs,filters:filters,fields:fields,resultExp:resultExp,types:r.types,dataFileType:guideConf.dataFileType,maxDataSize:guideConf.maxDataSize},
				success: function(data){
					if (data.indexOf('error:')==0) {
						alert(data.substring(6));
						return;
...
function reportCellDrill(reportConfName, topValues, leftValues, fieldValues, detail) {
	...
	for (var i=0; i<topValues.length; i++)
	{
		...
		if (dataSet.type == 6) {
			var whereConf = [{"level":1,"fieldInfo":transWhereInfo(conf2.tops[i].src,null,false),"oper":"等于","values":""+defaultEdit.getEditStyleValueByDisp(conf2.tops[i].srcEdit,topValues[i]),"disp":""}];
			conf2.tops[i].where = {conf:whereConf,exp:whereUtils.getExp(whereConf, "T1.", 1),disp:whereUtils.getDisp(whereConf)};	
		} else {
			var whereConf = [{"level":1,"fieldInfo":{disp:conf2.tops[i].name,dataType:conf2.tops[i].dataType,edit:conf2.tops[i].srcEdit,exp:conf2.tops[i].src,valueType:1,values:""},"oper":"等于","values":""+defaultEdit.getEditStyleValueByDisp(conf2.tops[i].srcEdit,topValues[i]),"disp":""}];
	...
	for (var i=0; i<leftValues.length; i++)
	{
		...
		if (dataSet.type == 6) {
			var whereConf = [{"level":1,"fieldInfo":transWhereInfo(conf2.lefts[i].src,null,false),"oper":"等于","values":""+defaultEdit.getEditStyleValueByDisp(conf2.lefts[i].srcEdit,leftValues[i]),"disp":""}]
			conf2.lefts[i].where = {conf:whereConf,exp:whereUtils.getExp(whereConf, "T1.", 1),disp:whereUtils.getDisp(whereConf)};	
		} else {
			var whereConf = [{"level":1,"fieldInfo":{disp:conf2.lefts[i].name,dataType:conf2.lefts[i].dataType,edit:conf2.lefts[i].srcEdit,exp:conf2.lefts[i].src,valueType:1,values:""},"oper":"等于","values":""+defaultEdit.getEditStyleValueByDisp(conf2.lefts[i].srcEdit,leftValues[i]),"disp":""}];
	...
	conf2.name = prex+"钻取";//"钻取结果" + (drillCounter++);
	var count = 1;
	while (aly.getRpx(conf2.name) != null){
		conf2.name = prex+"钻取_"+count;
		count++;
	}
...
function saveFile(content,type,needDataPath,oldDataPath) {
	...
	var dlg = art.dialog({
		id : dialogCount++,
		title : '保存'+type,
	    content: '<input id="saveGrpxName" type="text" value="'+guideConf.defaultSaveName+'" placeholder="文件名" style="width:150px;margin:5px 22px;">'
...
function openFile(type,callback) {
	...
	$.ajax({
		...
		async : false,
		url: contextPath + "/DLServletAjax?d="+new Date().getTime(),
		data: {action:2,oper:'getFiles',dfxFolderOnServer:guideConf.dfxFolderOnServer,qyxFolderOnServer:guideConf.qyxFolderOnServer,rpxFolderOnServer:guideConf.rpxFolderOnServer,inputFileFolderOnServer:guideConf.inputFileFolderOnServer,olapFolderOnServer:guideConf.olapFolderOnServer,fileDataFolderOnServer:guideConf.fileDataFolderOnServer},
		success: function(data){
			if (data.indexOf('error:')==0) {
				alert(data.substring(6));
				return;
	...
	var dlg = art.dialog({
		id : dialogCount++,
		title : '打开'+(type=='qyx'?'查询文件':(type=='olap'?'分析文件':(type=='fileData'?'数据文件':''))),
	    content: ''
	    	...
	    	+'<form id="openForm" METHOD=POST ACTION="'+contextPath+'/servlet/dataSphereServlet?action=38" ENCTYPE="multipart/form-data" target=hiddenFrame>'
	    		...
	    		+'<input type=hidden name=saveServer id=saveServer value="'+(type=='fileData'?1:0)+'">'
	    	+'</form>'
	    	+'</a>'
	...
	$('#openGrpxFile').change(function(){
		...
		if (type == 'qyx' || type == 'olap') {
			...
			if (f.indexOf('.txt') == -1 && f.indexOf('.csv') == -1 && f.indexOf('.xls') == -1 && f.indexOf('.xlsx') == -1) {
				alert("请选择[txt,csv,xls,xlsx]类型文件！");
				return;
			}
		}
		//alert(f);
	...
	var exists = type=='qyx'?existQyx:(type=='olap'?existOlap:(type=='fileData'?existFileDatas:[]));
...
function openDataFile(){
	openFile("fileData",function(content, file){
		...
		form.attr('action',url);
		form.attr('target', "_self");
		form.append('<input type="hidden" name="dfxScript">');
		form.find('input').val(file);
		$('body').append(form);
		form[0].submit();
	});
}
