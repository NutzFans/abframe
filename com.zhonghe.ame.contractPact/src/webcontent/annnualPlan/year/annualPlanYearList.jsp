...
<head>
<title>年度收入预算明细</title>
</head>
<body>
	<div style="width: auto; height: 99%; padding: 5px;">
		<div id="form1">
			<input class="nui-hidden" name="criteria._entity" value="com.zhonghe.ame.annualPlan.annualPlan.AnnualPlanYear" />
			<div class="nui-toolbar" style="border-bottom: 0; padding: 5px;">
				<table>
					<tr>
						...
						<td style="width: 70px; text-align: right;">数据来源:</td>
						<td style="width: 155px">
							<input name="criteria._expr[14].dataSourceType" class="nui-dictcombobox" dictTypeId="DATA_SOURCE_TYPE" showNullItem="true" nullItemText="全部" style="width: 150px" />
						</td>
					</tr>
					<tr>
		...
		<div class="nui-toolbar" style="border-bottom: 0; padding: 5px;">
			<table style="width: 100%;">
				<tr>
					<td>
						<a class="nui-button" id="annualPlanYear_add" iconCls="icon-add" onclick="add()">新增</a>
						<a class="nui-button" id="annualPlanYear_edit" iconCls="icon-edit" onclick="edit()">编辑</a>
						<a class="nui-button" id="annualPlanYear_del" iconCls="icon-remove" onclick="deleteInfo()">删除</a>
		...
		<div class="nui-fit">
			<div id="annualPlanYearGrid" sizeList="[500]" dataField="annualPlanYears" pageSize="500" class="nui-datagrid" style="width: 100%; height: 100%;"
				url="com.zhonghe.ame.annualPlan.annualPlan.queryAnnualPlanYear.biz.ext" idField="id" frozenStartColumn="0" frozenEndColumn="9" showSummaryRow="true" onshowrowdetail="onShowRowDetail"
	...
	<script type="text/javascript">
		...
		var grid = nui.get("annualPlanYearGrid");
		...
		function init() {
			// 按钮权限
			if (userId != 'sysadmin') {
				// 新增按钮 - annualPlanYear_add，编辑按钮 - annualPlanYear_edit，删除按钮 - annualPlanYear_del
				getOpeatorButtonAuth("annualPlanYear_add,annualPlanYear_edit,annualPlanYear_del");
			}

			var json = nui.encode({
				'loginUserId' : userId,
				'loginUserOrgId' : userOrgId,
				'authCode' : 'annualPlanYear'
			});
			nui.ajax({
		...
		function add() {
			nui.open({
				url : "/default/contractPact/annnualPlan/year/addAnnualPlanYear.jsp",
				width : '100%',
				height : '100%',
				title : "新增年度收入预算明细",
				onload : function() {
		...
		function edit() {
			...
			var data = row[0];
			nui.open({
				url : "/default/contractPact/annnualPlan/year/editAnnualPlanYear.jsp",
				width : '100%',
				height : '100%',
				title : "编辑年度收入预算明细",
				onload : function() {
			...
			if (row.length > 1 || row.length == 0) {
				...
				if (!confirm("是否删除？")) {
					...
					if (data) {
						...
						nui.ajax({
							url : "com.zhonghe.ame.annualPlan.annualPlan.delAnnualPlanYear.biz.ext",
							type : 'POST',
							data : json,
							contentType : 'text/json',
							success : function(o) {
		...
		function exportExcel() {
			...
			nui.ajax({
				url : "com.zhonghe.ame.annualPlan.annualPlan.exportAnnualPlanYearExcel.biz.ext",
				type : "post",
				data : json,
				cache : false,
				contentType : 'text/json',
		...
		function DATA_SOURCE_TYPE(e) {
			if (e.value == '1') {
				e.cellStyle = "color: blue";
			} else {
				e.cellStyle = "color: orange";
			}
			return nui.getDictText("DATA_SOURCE_TYPE", e.value);
		}
	</script>
