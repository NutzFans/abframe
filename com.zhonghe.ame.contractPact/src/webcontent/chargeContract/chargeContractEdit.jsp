				...
				<div style="padding: 5px;">
					<table style="table-layout: fixed;">
						...
						<tr>
							...
							<td>
								<input id="tenderId" name="tenderId" class="nui-buttonedit" onbuttonclick="bidInfoSelect" style="width: 200px" required="false" enabled="true" allowInput="false" />
							</td>
							<td align="right" style="width: 130px">投资额(万元)：</td>
							<td>
								<input name="investAmount" id="investAmount" class="nui-textbox" style="width: 200px" required="true" emptyText="投标记录关联带出" />
							</td>
						</tr>
						<tr>
			...
			if (chargePlans.length > 0) {
				...
				for (var i = 0; i < chargePlansJson.length; i++) {
					...
					if (isNotNum(chargePlansJson[i].jan) || isNotNum(chargePlansJson[i].feb) || isNotNum(chargePlansJson[i].mar) || isNotNum(chargePlansJson[i].apr)
							|| isNotNum(chargePlansJson[i].may) || isNotNum(chargePlansJson[i].jun) || isNotNum(chargePlansJson[i].jul) || isNotNum(chargePlansJson[i].aug)
							|| isNotNum(chargePlansJson[i].sep) || isNotNum(chargePlansJson[i].oct) || isNotNum(chargePlansJson[i].nov) || isNotNum(chargePlansJson[i].dec)) {
