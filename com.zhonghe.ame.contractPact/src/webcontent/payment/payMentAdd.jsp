...
<body>
	<div class="nui-fit">
		<fieldset id="field1"
			style="border: solid 1px #aaa; padding: 3px; width: 98%;">
			<legend>发起付款流程信息</legend>
			<form id="form1" method="post">
				<input name="files" id="fileids" class="nui-hidden" /> <input
					name="implementOrg" id="implementOrg" class="nui-hidden" />
					...
					<table style="table-layout: fixed;">
						<tr>
							...
							<td><input id="createTime" name="createTime"
								class="nui-datepicker" style="width: 300px" readonly="readonly" /></td>
						</tr>
						<tr>
							<td align="right" style="width: 160px">合同编号：</td>
							<td><input id="contractId" name="contractId"
						...
						<tr>
							...
							<td><input id="contractNature" name="contractNature"
								class="nui-dictcombobox" dictTypeId="CONTRACT_NATURE" style="width: 100%" required="true"
								enabled="false" /></td>
							...
							<td><input id="payer" name="payer" class="nui-dictcombobox"
								dictTypeId="PAYER" style="width: 100%" required="true"
								enabled="false" /></td>
						...
						<tr>
							...
							<td><input id="paidContractSum" name="paidContractSum"
								class="nui-textbox" onvaluechanged="editPaidContractSum"
								style="width: 300px" required="false" enabled="false" /></td>
							<td align="right" style="width: 160px">本次支付金额(元)：</td>
							<td><input id="applyPayContractSum"
								name="applyPayContractSum"
								onvaluechanged="editApplyPayContractSum" class="nui-textbox"
						...
						<tr>
							...
							<td><input id="invoiceType" name="invoiceType"
								class="nui-dictcombobox" dictTypeId="MIS_MA_INVOICETYPE"
								style="width: 300px" required="true" enabled="true" /></td>
							<td align="right" style="width: 160px">本次付款进度：</td>
							<td><input id="payType" name="payType"
								class="nui-dictcombobox" dictTypeId="payType"
								style="width: 300px" required="true" enabled="true" /></td>
							<td align="right" style="width: 160px">最晚付款日期：</td>
							<td><input id="endTime" name="endTime"
								class="nui-datepicker" style="width: 300px" required="true" /></td>
						</tr>
						<tr>
							<td align="right">收款单位：</td>
							<td colspan="8"><input name="signatory" id="signatory"
								class="nui-hidden" style="width: 300px" /><input id="signatoryname" name="signatoryname"
								class="nui-textbox" style="width: 100%" required="true"
								enabled="false" /></td>
						</tr>
						<tr>
							<td align="right">收款单位开户行名称：</td>
							<td colspan="8"><input id="accountName" name="accountName"
								class="nui-textbox" style="width: 100%" required="true" /></td>
						</tr>
						<tr>
							<td align="right">收款单位开户行账号：</td>
							<td colspan="8"><input id="account" name="account" class="nui-textbox"
								style="width: 100%" required="true" /></td>
						</tr>
						<tr>
							<td align="right">支付依据及情况说明：</td>
							<td colspan="8"><input id="remark" name="remark"
								class="nui-textarea" style="width: 100%; height: 150px"
								required="true" /></td>
						</tr>
					</table>
				</div>
	...
	<div style="text-align: center; padding: 8px;" class="nui-toolbar">
		<a class="nui-button" onclick="onOk(0)" id="saveFeame"
			...
			class="nui-button" onclick="closeCancel" iconCls="icon-close"
	...
	<script type="text/javascript">
		...
		function setViewData(data) {
			...
			if (null != data.contractSum) {
				...
				nui.get("actContractSumCapital").setValue(
						numberParseChina(data.contractSum));
			}
			if (null != data.id) {
						...
						.ajax({
							...
							success : function(text) {
								for (var i = 0; i < text.data.length; i++) {
									payAll = payAll
											+ parseFloat(text.data[i].applyPayContractSum);
								}
								//alert(payAll.toFixed(2));
								nui.get("paidContractSum").setValue(
										payAll.toFixed(2));
								nui.get("paidContractSumCapital").setValue(
										numberParseChina(payAll.toFixed(2)));

								//alert(isNaN(payAll.toFixed(2)));

								if (isNaN(payAll.toFixed(2))
										|| payAll.toFixed(2) == 0.00) {
				...
				nui
						.ajax({
							...
							success : function(text) {
								...
								nui.get("implementOrg").setValue(
										text.data[0].implementOrg);
							},
							error : function(jqXHR, textStatus, errorThrown) {
								alert(jqXHR.responseText);
							}
						...
						ondestroy : function(action) {
							if (action == "ok") {
								...
								if (data) {
									...
									nui.get("contractName").setValue(
											data.contractName);
									nui.get("contractSum").setValue(
											data.contractSum);
									nui.get("signatoryname").setValue(
											data.signatoryname);
									nui.get("signatory").setValue(
											data.signatory);
									nui.get("contractType").setValue(
											data.contractType);	
									nui.get("contractNature").setValue(
											data.contractNature);	
									nui.get("payer").setValue(
											data.payer);		
