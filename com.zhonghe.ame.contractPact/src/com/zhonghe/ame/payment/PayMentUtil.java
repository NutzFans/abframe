...
public class PayMentUtil {
	...
	public String checkPayPaid(String payId, String applyPayContractSum) {
		try {
			...
			if (ObjectUtil.isNotNull(entity)) {
				...
				if (quater == 1) {
					return NumberUtil.isLessOrEqual(new BigDecimal(applyPayContractSum), oneQuater) ? "1" : "2";
				}
				if (quater == 2) {
					return NumberUtil.isLessOrEqual(new BigDecimal(applyPayContractSum), twoQuater) ? "1" : "2";
				}
				if (quater == 3) {
					return NumberUtil.isLessOrEqual(new BigDecimal(applyPayContractSum), threeQuater) ? "1" : "2";
				}
				if (quater == 4) {
					return NumberUtil.isLessOrEqual(new BigDecimal(applyPayContractSum), fourQuater) ? "1" : "2";
				}
				return "4";
			} else {
				return "3";
