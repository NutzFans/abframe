	...
	<div class="nui-fit">
		<div class="nui-splitter" style="width:100%;height:100%;">
			...
			<div size="45" showCollapseButton="true" style="background-color: ;"> 
				<br/>
				<!-- 机构树展现  start-->
				<ul id="tree1" class="nui-tree" url="com.primeton.eos.common.departmentClassifyType.getdepartmentCls.biz.ext" 
			...
			<div size="190" showCollapseButton="true" > 
				<div id="body_tab" class="nui-tabs" activeIndex="0" style="width:auto;height:100%" >
					<!-- 机构详情信息 -->
					<div id="detail" url="../departClassifyAll/departmentClsDetail.jsp" title="费用类型详情" refreshOnClick="true">
					</div>
					
					<!-- 下级机构列表 -->
					 <!-- <div id="list" url="../feetypetree/feetypeList.jsp" title="下级费用类型" >
					</div> --> 
				</div>
			</div>
		</div>
...
<script type="text/javascript">
		...
		function search(){
			var json = nui.encode({});
			nui.ajax({
            	url: "com.primeton.eos.common.departmentClassifyType.getdepartmentCls.biz.ext",
            	data: json,
            	type: 'POST',
            	cache: false,
		...
		function click(e){
			var deptexptypeid = e.node.exptypeid;
			...
			for(var i=0;i<tabs.tabs.length;i++){
				...
				if(temp_tab.title=="费用类型详情"){
					tabs.loadTab("../departClassifyAll/departmentClsDetail.jsp?deptexptypeid="+deptexptypeid+"&type="+type, temp_tab);
				}else{
					//tabs.loadTab("../feetypetree/feetypeList.jsp?feeseq="+feeseq+"&pName="+pName, temp_tab);
				}
			}
		}
		...
		function onAddFromOrg(){
			var node = tree.getSelectedNode();
			nui.open({
		            url: "<%=request.getContextPath() %>/mpcommon/departClassifyAll/departmenttypeAddChose.jsp",
		            title: "新增部门费用类型信息", 
		            width: 730,
		            height: 230,
		            allowResize:false,
		...
		function onEditNode(){
			...
			var exptypeid = node.exptypeid;
			var type = node.type;
			var exptypename = freshNode.exptypename;
			nui.open({
	            url: "<%=request.getContextPath() %>/mpcommon/departClassifyAll/modifyDepartFeeType.jsp?exptypeid="+exptypeid+"&type="+type,
	            title: "修改费用类型信息", 
	            width: 730,
	            height: 255,
	            allowResize:false,
		...
		function onRemoveNode(){
			...
			nui.ajax({
            url: "com.primeton.eos.common.departmentClassifyType.checkRelationVsBudget.biz.ext",
            type: 'POST',
            data: json1,
            cache: false,
            contentType:'text/json',
            success: function (text) {
                ...
                if(statuss=="0"){ 
					nui.confirm("确定要删除该费用类型?","提示",
						function (action) {
						   if (action == "ok") {
						   		var json = nui.encode({departfeetype:{"deptexptypeid":node.exptypeid},type:node.type});
			        			nui.ajax({
					            	url: "com.primeton.eos.common.departmentClassifyType.deleteDepartFeeType.biz.ext",
					            	data: json,
					            	type: 'POST',
					            	cache: false,
					            	contentType: 'text/json',
